            ...
            <v-form ref="form" lazy-validation>
              ...
              <v-card-text>
                <v-text-field label="Designation" prepend-inner-icon="extension" dense
                  :rules="[(v) => !!v || 'Ce champ est requis']" outlined v-model="svData.name_categorie_rubrique"></v-text-field>
              </v-card-text>
              <v-card-actions>
                <v-spacer></v-spacer>
...
export default {
  ...
  methods: {
    ...
    testTitle() {
      if (this.edit == true) {
        this.titleComponent = "modification de " + item.name_categorie_rubrique;
      } else {
        this.titleComponent = "Ajout Categorie";
      }
    }
    ,
    onPageChange() {
      this.fetch_data(`${this.apiBaseURL}/fetch_all_categorie_rubrique_pers?page=`);
    },
    ...
    validate() {
      if (this.$refs.form.validate()) {
        ...
        this.insertOrUpdate(
          `${this.apiBaseURL}/insert_categorie_rubrique_pers`,
    ...
    editData(id) {
      this.editOrFetch(`${this.apiBaseURL}/fetch_single_categorie_rubrique_pers/${id}`).then(
        ({ data }) => {
          var donnees = data.data;
    ...
    clearP(id) {
      this.confirmMsg().then(({ res }) => {
        this.delGlobal(`${this.apiBaseURL}/delete_categorie_rubrique_pers/${id}`).then(
          ({ data }) => {
