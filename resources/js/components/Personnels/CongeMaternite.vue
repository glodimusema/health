                      ...
                      <v-card-text>
                        ...
                        <v-layout row wrap>
                          ...
                          <v-flex xs12 sm12 md12 lg12>
                            <div class="mr-1">
                              <v-text-field type="date" label="Date Accouchement" prepend-inner-icon="event" dense
                                :rules="[(v) => !!v || 'Ce champ est requis']" outlined v-model="svData.dateAccouchement">
                              </v-text-field>
                            </div>
                          </v-flex>
                          ...
                          <v-flex xs12 sm12 md12 lg12>
                            <div class="mr-1">
                              <v-text-field label="Mode Accouchement" prepend-inner-icon="event" dense
                          ...
                          <template v-slot:default>
                            <thead>
                              <tr>
                                ...
                                <th class="text-left">DateAccouch.</th>
                                <th class="text-left">ModeAccouch.</th>
                                <th class="text-left">AutresDet.</th>
                            ...
                            <tbody>
                              <tr v-for="item in fetchData" :key="item.id">
                                ...
                                <td>{{ item.dateAccouchement }}</td>
                                <td>{{ item.modeAccouchement }}</td>
                                <td>{{ item.autresDetail }}</td>
                                <td>{{ item.annexeMaternite }}</td>
                                <td>
                                  ...
                                  <v-menu bottom rounded offset-y transition="scale-transition">
                                    ...
                                    <v-list dense width="">
                                      ...
                                      <v-list-item link @click="printBill(item.annexeMaternite)">
                                        <v-list-item-icon>
...
export default {
  ...
  data() {
    return {
      ...
      // //id,refEnteteConge,dateAccouchement,modeAccouchement,autresDetail,annexeMaternite,author
      svData: {
        id: '',        
        refEnteteConge: 0,
        dateAccouchement:'',
  ...
  methods: {
    ...
    updatePhoto() {
      ...
      if (this.edit == true) {
        ...
        axios
          .post(`${this.apiBaseURL}/update_maternite`, formData, config)
          .then(({ data }) => {
            this.image = "";
        ...
        this.svData.author = this.userData.name;
        axios
          .post(`${this.apiBaseURL}/insert_maternite`, formData, config)
          .then(({ data }) => {
            this.image = "";
            this.showMsg(data.data);
    ...
    printFiche(id) {
      window.open(`${this.apiBaseURL}/pdf_conge_maternite?id=` + id);
    },

    editData(id) {
      this.editOrFetch(`${this.apiBaseURL}/fetch_single_maternite/${id}`).then(
        ({ data }) => {
          var donnees = data.data;
          donnees.map((item) => {
            this.svData.id = item.id;
            this.svData.refEnteteConge = item.refEnteteConge;
            this.svData.dateAccouchement = item.dateAccouchement; 
    ...
    deleteData(id) {
      this.confirmMsg().then(({ res }) => {
        this.delGlobal(`${this.apiBaseURL}/delete_maternite/${id}`).then(
          ({ data }) => {
            this.showMsg(data.data);
            this.fetchDataList();
          }
    ...
    fetchDataList() {
      this.fetch_data(`${this.apiBaseURL}/fetch_maternite/${this.refEnteteConge}?page=`);
      //
    }
