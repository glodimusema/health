        ...
        <v-dialog v-model="dialog" max-width="500px" scrollable transition="dialog-bottom-transition">
          <v-card :loading="loading">
            <v-form ref="form" lazy-validation>
              ...
              <v-card-text>
                <v-text-field label="Menu" prepend-inner-icon="extension" dense
                  :rules="[(v) => !!v || 'Ce champ est requis']" outlined v-model="svData.name_menu"></v-text-field>

                  <v-text-field label="N° Menu" prepend-inner-icon="extension" dense
                  :rules="[(v) => !!v || 'Ce champ est requis']" outlined v-model="svData.numero_menu"></v-text-field>
...
export default {
  ...
  methods: {
    ...
    testTitle() {
      if (this.edit == true) {
        this.titleComponent = "modification de " + item.name_menu;
      } else {
        this.titleComponent = "Ajout Menu";
      }
    },
    ...
    ,
    onPageChange() {
      this.fetch_data(`${this.apiBaseURL}/fetch_liste_menu?page=`);
    },
    ...
    validate() {
      if (this.$refs.form.validate()) {
        ...
        this.insertOrUpdate(
          `${this.apiBaseURL}/insert_liste_menu`,
    ...
    editData(id) {
      this.editOrFetch(`${this.apiBaseURL}/fetch_single_liste_menu/${id}`).then(
        ({ data }) => {
          var donnees = data.data;
    ...
    clearP(id) {
      this.confirmMsg().then(({ res }) => {
        this.delGlobal(`${this.apiBaseURL}/delete_liste_menu/${id}`).then(
          ({ data }) => {
