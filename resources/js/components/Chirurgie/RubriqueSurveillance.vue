<template>
              ...
              <v-form ref="form" lazy-validation>
                ...
                <v-card-text>
                  <v-text-field
                    ...
                    outlined
                    v-model="svData.nom_rubliquesurv"
                  ></v-text-field>
                </v-card-text>
                <v-card-actions>
                  <v-spacer></v-spacer>
  ...
  export default {
    ...
    methods: {
      ...
      showModal() {
        this.dialog = true;
        this.titleComponent = "Ajout Rubrique Surveillance";
        this.edit = false;
      ...
      testTitle() {
        if (this.edit == true) {
          this.titleComponent = "modification de " + item.nom_rubliquesurv;
        } else {
          this.titleComponent = "Ajout Rubrique Surveillance";
        }
      }
      ,
      onPageChange() {
        this.fetch_data(`${this.apiBaseURL}/fetch_rubriquesurveillance?page=`);
      },
      ...
      validate() {
        if (this.$refs.form.validate()) {
          ...
          this.insertOrUpdate(
            `${this.apiBaseURL}/insert_rubriquesurveillance`,
      ...
      editData(id) {
        this.editOrFetch(`${this.apiBaseURL}/fetch_single_rubriquesurveillance/${id}`).then(
          ({ data }) => {
            var donnees = data.data;
      ...
      clearP(id) {
        this.confirmMsg().then(({ res }) => {
          this.delGlobal(`${this.apiBaseURL}/delete_rubriquesurveillance/${id}`).then(
            ({ data }) => {
