                ...
                <v-dialog v-model="dialog" max-width="1000px" persistent>
                  <v-card :loading="loading">
                    <v-form ref="form" lazy-validation>
                      <v-card-title>
                        Fiche de Consentement <v-spacer></v-spacer>
                      ...
                      <v-card-text>
                        ...
                        <v-layout row wrap>
                          ...
                          <v-flex xs12 sm12 md6 lg6>
                            <div class="mr-1">
                              <v-autocomplete label="Type Intervention" prepend-inner-icon="home"
                                :rules="[(v) => !!v || 'Ce champ est requis']" :items="stataData.InterventionList"
                                item-text="nom_intervention" item-value="id" dense outlined v-model="svData.intervention" chips
                                clearable>
                              </v-autocomplete>
                            </div>
                          </v-flex>
                          ...
                          <v-flex xs12 sm12 md6 lg6>
                            <div class="mr-1">
                              <v-text-field type="date" label="Date Consentement" prepend-inner-icon="event" dense
                                :rules="[(v) => !!v || 'Ce champ est requis']" outlined v-model="svData.dateConsentement">
                              </v-text-field>
                            </div>
                          </v-flex>             
                          ...
                          <v-flex xs12 sm12 md6 lg6>
                            <div class="mr-1">
                              <v-textarea label="Prévision" prepend-inner-icon="draw" dense
                                :rules="[(v) => !!v || 'Ce champ est requis']" outlined v-model="svData.prevision">
                              </v-textarea>
                            </div>
                          </v-flex>
                          ...
                          <v-flex xs12 sm12 md6 lg6>
                            <div class="mr-1">
                              <v-textarea label="Actes Chirurgical" prepend-inner-icon="draw" dense
                                :rules="[(v) => !!v || 'Ce champ est requis']" outlined v-model="svData.actechirurgie">
                              </v-textarea>
                            </div>
                          </v-flex>
...
export default {
  ...
  methods: {
    ...
    validate() {
      if (this.$refs.form.validate()) {
        ...
        if (this.edit) {
          ...
          this.insertOrUpdate(
            `${this.apiBaseURL}/update_consentement/${this.svData.id}`,
          ...
          this.svData.author = this.userData.name;
          this.insertOrUpdate(
            `${this.apiBaseURL}/insert_consentement`,
            JSON.stringify(this.svData)
          )
            .then(({ data }) => {
              this.showMsg(data.data);
    ...
    editData(id) {
      this.editOrFetch(`${this.apiBaseURL}/fetch_single_consentement/${id}`).then(
        ({ data }) => {
          var donnees = data.data;
          donnees.map((item) => {
            this.svData.id = item.id;
            this.svData.refEnteteOpe = item.refEnteteOpe;
            this.svData.dateConsentement = item.dateConsentement;
            this.svData.chirurgien = item.chirurgien;
            this.svData.anesthesiste = item.anesthesiste;
            this.svData.intervention = item.intervention;
            this.svData.prevision = item.prevision;
            this.svData.actechirurgie = item.actechirurgie;
    ...
    deleteData(id) {
      this.confirmMsg().then(({ res }) => {
        this.delGlobal(`${this.apiBaseURL}/delete_consentement/${id}`).then(
          ({ data }) => {
            this.showMsg(data.data);
            this.fetchDataList();
          }
    ...
    fetchDataList() {
      this.fetch_data(`${this.apiBaseURL}/fetch_consentement_detailoperation/${this.refEnteteOpe}?page=`);
    }
