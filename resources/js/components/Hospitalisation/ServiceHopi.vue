<template>
              ...
              <v-form ref="form" lazy-validation>
                ...
                <v-card-text>
                  <v-text-field
                    ...
                    :rules="[(v) => !!v || 'Ce champ est requis']" dense
                    outlined
                    v-model="svData.nom_servicehospi"
                  ></v-text-field>

                  <v-text-field type="number" label="Prix du Service($)" prepend-inner-icon="event" dense
                        :rules="[(v) => !!v || 'Ce champ est requis']" outlined v-model="svData.prix_servicehospi">
                  </v-text-field>
  ...
  export default {
    ...
    methods: {
      ...
      showModal() {
        this.dialog = true;
        this.titleComponent = "Ajout Service Hospitalisation";
        this.edit = false;
      ...
      testTitle() {
        if (this.edit == true) {
          this.titleComponent = "modification de " + item.nom_servicehospi;
        } else {
          this.titleComponent = "Ajout Service ";
        }
      }
      , 
    
      onPageChange() {
        this.fetch_data(`${this.apiBaseURL}/fetch_servicehospi?page=`);
      },
      ...
      validate() {
        ...
        if (this.$refs.form.validate()) {
          ...
          this.insertOrUpdate(
            `${this.apiBaseURL}/insert_servicehospi`,
      ...
      clearP(id) {
        this.confirmMsg().then(({ res }) => {
          this.delGlobal(`${this.apiBaseURL}/delete_servicehospi/${id}`).then(
            ({ data }) => {
