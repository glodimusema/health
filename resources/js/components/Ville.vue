<template>
  ...
  export default {
    ...
    methods: {
      ...
      showModal() {
        this.dialog = true;
        this.titleComponent = "Ajout ville ";
        this.edit = false;
      ...
      // ,
      onPageChange() {
        this.fetch_data(`${this.apiBaseURL}/ad1/fetch_ville?page=`);
      },
      ...
      validate() {
        if (this.$refs.form.validate()) {
          ...
          this.insertOrUpdate(
            `${this.apiBaseURL}/ad1/insert_ville`,
      ...
      editData(id) {
        this.editOrFetch(`${this.apiBaseURL}/ad1/fetch_single_ville/${id}`).then(
          ({ data }) => {
            var donnees = data.data;
  
            donnees.map((item) => {
              this.titleComponent = "modification de " + item.nomVille;
      ...
      clearP(id) {
        this.confirmMsg().then(({ res }) => {
          this.delGlobal(`${this.apiBaseURL}/ad1/delete_ville/${id}`).then(
            ({ data }) => {
