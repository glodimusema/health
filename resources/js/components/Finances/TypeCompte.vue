<template>
  ...
  export default {
    ...
    methods: {
      ...
      showModal() {
        this.dialog = true;
        this.titleComponent = "Ajout Type Compte ";
        this.edit = false;
  ...
  //
      testTitle() {
        if (this.edit == true) {
          this.titleComponent = "modification de " + item.nom_typecompte;
        } else {
    ...
    //   }, 300),
      onPageChange() {
        this.fetch_data(`${this.apiBaseURL}/fetch_typecompte?page=`);
      },
      ...
      validate() {
        ...
        if (this.$refs.form.validate()) {
          this.isLoading(true);          
          this.insertOrUpdate(
            `${this.apiBaseURL}/insert_typecompte`,
      ...
      editData(id) {
        this.editOrFetch(`${this.apiBaseURL}/fetch_single_typecompte/${id}`).then(
          ({ data }) => {
            var donnees = data.data;
      ...
      clearP(id) {
        this.confirmMsg().then(({ res }) => {
          this.delGlobal(`${this.apiBaseURL}/delete_typecompte/${id}`).then(
            ({ data }) => {
