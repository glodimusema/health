<template>
  ...
  export default {
    ...
    methods: {
      ...
      showModal() {
        this.dialog = true;
        this.titleComponent = "Ajout Mode Paiement ";
        this.edit = false;
    ...
    //   }, 300),
      onPageChange() {
        this.fetch_data(`${this.apiBaseURL}/fetch_modepaie?page=`);
      },
      fetchAccess() {
      ...
      validate() {
        if (this.$refs.form.validate()) {
          ...
          this.insertOrUpdate(
            `${this.apiBaseURL}/insert_modepaie`,
      ...
      editData(id) {
        this.editOrFetch(`${this.apiBaseURL}/fetch_single_modepaie/${id}`).then(
          ({ data }) => {
            var donnees = data.data;
      ...
      clearP(id) {
        this.confirmMsg().then(({ res }) => {
          this.delGlobal(`${this.apiBaseURL}/delete_modepaie/${id}`).then(
            ({ data }) => {
