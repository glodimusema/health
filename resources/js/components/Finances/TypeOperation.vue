<template>
              ...
              <v-form ref="form" lazy-validation>
                ...
                <v-card-text>
                  <v-text-field
                    ...
                    outlined
                    v-model="svData.nom_typeoperation"
                  ></v-text-field>
                </v-card-text>
                <v-card-actions>
                  <v-spacer></v-spacer>
  ...
  export default {
    ...
    methods: {
      ...
      showModal() {
        this.dialog = true;
        this.titleComponent = "Ajout Type Operation ";
        this.edit = false;
      ...
      testTitle() {
        if (this.edit == true) {
          this.titleComponent = "modification de " + item.nom_typeoperation;
        } else {
    ...
    //   }, 300),
      onPageChange() {
        this.fetch_data(`${this.apiBaseURL}/fetch_typeoperation?page=`);
      },
      fetchAccess() {
      ...
      validate() {
        ...
        if (this.$refs.form.validate()) {
          this.isLoading(true);          
          this.insertOrUpdate(
            `${this.apiBaseURL}/insert_typeoperation`,
      ...
      editData(id) {
        this.editOrFetch(`${this.apiBaseURL}/fetch_single_typeoperation/${id}`).then(
          ({ data }) => {
            var donnees = data.data;
      ...
      clearP(id) {
        this.confirmMsg().then(({ res }) => {
          this.delGlobal(`${this.apiBaseURL}/delete_typeoperation/${id}`).then(
            ({ data }) => {
