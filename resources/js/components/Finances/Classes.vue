            ...
            <v-form ref="form" lazy-validation>
              ...
              <v-card-text>
                <v-text-field label="Designation" prepend-inner-icon="extension" :rules="[(v) => !!v || 'Ce champ est requis']" outlined
                  v-model="svData.nom_classe"></v-text-field>

                  <v-text-field label="Numéro Classe" prepend-inner-icon="extension" :rules="[(v) => !!v || 'Ce champ est requis']" outlined
                  v-model="svData.numero_classe"></v-text-field>
              </v-card-text>
              <v-card-actions>
                <v-spacer></v-spacer>
              ...
              <template v-slot:default>
                <thead>
                  <tr>
                    ...
                    <th class="text-left">N°Classe</th>
...
export default {
  ...
  methods: {
    ...
    testTitle() {
      if (this.edit == true) {
        this.titleComponent = "modification de " + item.nom_classe;
      } else {
  ...
  //   }, 300),
    onPageChange() {
      this.fetch_data(`${this.apiBaseURL}/fetch_classecompte?page=`);
    },
    ...
    validate() {
      ...
      if (this.$refs.form.validate()) {
        this.isLoading(true);          
        this.insertOrUpdate(
          `${this.apiBaseURL}/insert_classecompte`,
    ...
    editData(id) {
      this.editOrFetch(`${this.apiBaseURL}/fetch_single_classecompte/${id}`).then(
        ({ data }) => {
          var donnees = data.data;
    ...
    clearP(id) {
      this.confirmMsg().then(({ res }) => {
        this.delGlobal(`${this.apiBaseURL}/delete_classecompte/${id}`).then(
          ({ data }) => {
