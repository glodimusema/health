<template>
              ...
              <v-form ref="form" lazy-validation>
                ...
                <v-card-text>
                  <v-text-field
                    ...
                    outlined
                    v-model="svData.name_categorie_societe"
                  ></v-text-field>
                </v-card-text>
                <v-card-actions>
                  <v-spacer></v-spacer>
  ...
  export default {
    ...
    methods: {
      ...
      showModal() {
        this.dialog = true;
        this.titleComponent = "Ajout Catégorie Société ";
        this.edit = false;
      ...
      testTitle() {
        if (this.edit == true) {
          this.titleComponent = "modification de " + item.name_categorie_societe;
        } else {
    ...
    //   }, 300),
      onPageChange() {
        this.fetch_data(`${this.apiBaseURL}/fetch_categorie_societe?page=`);
      },
      ...
      validate() {
        if (this.$refs.form.validate()) {
          ...
          this.insertOrUpdate(
            `${this.apiBaseURL}/insert_categorie_societe`,
      ...
      editData(id) {
        this.editOrFetch(`${this.apiBaseURL}/fetch_single_categorie_societe/${id}`).then(
          ({ data }) => {
            var donnees = data.data;
      ...
      clearP(id) {
        this.confirmMsg().then(({ res }) => {
          this.delGlobal(`${this.apiBaseURL}/delete_categorie_societe/${id}`).then(
            ({ data }) => {
