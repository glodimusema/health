            ...
            <v-form ref="form" lazy-validation>
              ...
              <v-card-text>
                ...
                <v-layout row wrap>
                  ...
                  <div class="mr-1">
                    <v-text-field label="Designation Produit " prepend-inner-icon="event" dense
                      :rules="[(v) => !!v || 'Ce champ est requis']" outlined v-model="svData.nom_produit">
                    </v-text-field>
                  </div>
                </v-flex>
                ...
                <v-flex xs12 sm12 md12 lg12>
                  <div class="mr-1">
                    <v-text-field type="number" label="Prix Unitaire " prepend-inner-icon="event" dense
                      :rules="[(v) => !!v || 'Ce champ est requis']" outlined v-model="svData.prix_produit">
                    </v-text-field>
                  </div>
                </v-flex>
                ...
                <v-flex xs12 sm12 md12 lg12>
                  <div class="mr-1">
                    <v-text-field label="Code Produit " prepend-inner-icon="event" dense
                      :rules="[(v) => !!v || 'Ce champ est requis']" outlined v-model="svData.code_produit">
                    </v-text-field>
                  </div>
                </v-flex>
                ...
                <v-flex xs12 sm12 md12 lg12>
                  <div class="mr-1">
                    <v-autocomplete label="Selectionnez la Catégorie" prepend-inner-icon="mdi-map" dense
                      ...
                      item-text="name_categorie_societe" item-value="id" outlined v-model="svData.refCategorieSociete">
                    </v-autocomplete>
                  </div>
                </v-flex>
                  ...
                  <template v-slot:default>
                    <thead>
                      <tr>
                        ...
                        <th class="text-left">TypeProduit</th>
  ...
  methods: {
    ...
    validate() {
      if (this.$refs.form.validate()) {
        ...
        if (this.edit) {
          ...
          this.insertOrUpdate(
            `${this.apiBaseURL}/update_produitfin/${this.svData.id}`,
          ...
          this.svData.author = this.userData.name;
          this.insertOrUpdate(
            `${this.apiBaseURL}/insert_produitfin`,
            JSON.stringify(this.svData)
          )
            .then(({ data }) => {
              this.showMsg(data.data);
      ...
      this.editOrFetch(`${this.apiBaseURL}/fetch_single_produit/${id}`).then(
        ({ data }) => {
          ...
          donnees.map((item) => {
            ...
            this.svData.nom_produit = item.nom_produit;
            this.svData.prix_produit = item.prix_produit;
            this.svData.code_produit = item.code_produit;
    ...
    deleteData(id) {
      this.confirmMsg().then(({ res }) => {
        this.delGlobal(`${this.apiBaseURL}/delete_produitfin/${id}`).then(
          ({ data }) => {
            this.showMsg(data.data);
            this.fetchDataList();
          }
    ...
    fetchDataList() {
      this.fetch_data(`${this.apiBaseURL}/fetch_produitfin?page=`);
