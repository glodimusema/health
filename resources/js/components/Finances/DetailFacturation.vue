    ...
    <v-dialog v-model="etatModal" persistent max-width="1200px">
      <v-card>
        ...
        <v-card-title class="red">
          ...
          <v-btn color="#FFFFFF" @click="showCreatePaiement()" text fab depressed>
            <v-icon>mdi-cash-100</v-icon>
          </v-btn>
          <v-btn color="#FFFFFF" @click="printInvoice()" text fab depressed>
            <v-icon>print</v-icon>
          </v-btn>
              ...
              <v-dialog v-model="dialog" max-width="600px" persistent>
                <v-card :loading="loading">
                  <v-form ref="form" lazy-validation>
                    ...
                    <v-card-text>
                      <v-textarea v-if="examens != '  --  ' || examens != '' || examens != '--'" readonly type="text" label="Les Examens demandés par le Medecin : " prepend-inner-icon="event"
                      ...
                      <v-flex xs12 sm12 md12 lg12>
                        <div class="mr-1">
                          <v-autocomplete label="Selectionnez le Type de Produit" prepend-inner-icon="map"
                            ...
                            clearable chips
                            @change="get_produit_for_typeproduit(svData.refTypeProduit, organisationAbonne)">
                          </v-autocomplete>
                        </div>
                      </v-flex>
                      ...
                      <v-text-field type="number" label="Prix Unitaire ($) " prepend-inner-icon="event" dense outlined
                        v-model="svData.prixunitaire">
                      </v-text-field>
        ...
        <v-card :loading="loading">
          <v-form ref="form" lazy-validation>
            ...
            <v-card-actions>
              ...
              <v-btn color="#B72C2C" dark :loading="loading" @click="validate_paie">
                {{ edit ? "Modifier" : "Ajouter" }}
              </v-btn>
            </v-card-actions>
          </v-form>
                                ...
                                <v-menu bottom rounded offset-y transition="scale-transition">
                                  ...
                                  <v-list dense width="">
                                    ...
                                    <v-list-item v-if="item.categoriemaladiemvt == 'PRIVE(E)'" link
                                      @click="printFacturePriveeGF(item.refEnteteFacturation)">
                                      <v-list-item-icon>
                                        <v-icon color="#B72C2C">print</v-icon>
                                      </v-list-item-icon>
                                      <v-list-item-title style="margin-left: -20px">Facture des Privé(e)s
                                        (A4)</v-list-item-title>
                                    </v-list-item>

                                    <v-list-item v-if="item.categoriemaladiemvt == 'PRIVE(E)'" link
                                      @click="printFacturePriveePF(item.refEnteteFacturation)">
                                      <v-list-item-icon>
                                        <v-icon color="#B72C2C">print</v-icon>
                                      </v-list-item-icon>
                                      <v-list-item-title style="margin-left: -20px">Facture des Privé(e)s
                                    ...
                                    <v-list-item v-if="item.categoriemaladiemvt == 'ABONNE(E)'" link
                                      @click="printFactureAbonneeGF(item.refEnteteFacturation)">
                                      <v-list-item-icon>
                                        <v-icon color="#B72C2C">print</v-icon>
                                      </v-list-item-icon>
                                      <v-list-item-title style="margin-left: -20px">Facture des Abonné(e)s
                                        (A4)</v-list-item-title>
                                    </v-list-item>

                                    <v-list-item v-if="item.categoriemaladiemvt == 'ABONNE(E)'" link
                                      @click="printFactureAbonneePF(item.refEnteteFacturation)">
                                      <v-list-item-icon>
                                        <v-icon color="#B72C2C">print</v-icon>
                                      </v-list-item-icon>
                                      <v-list-item-title style="margin-left: -20px">Facture des Abonné(e)s
                                    ...
                                    <v-list-item v-if="item.categoriemaladiemvt == 'PRIVE(E)'" link
                                      @click="printFacturePriveeGF_Synthese(item.refEnteteFacturation)">
                                      <v-list-item-icon>
                                        <v-icon color="#B72C2C">print</v-icon>
                                      </v-list-item-icon>
                                      <v-list-item-title style="margin-left: -20px">Facture Synthèse des Privé(e)s
                                        (A4)</v-list-item-title>
                                    </v-list-item>

                                    <v-list-item v-if="item.categoriemaladiemvt == 'PRIVE(E)'" link
                                      @click="printFacturePriveePF_Synthese(item.refEnteteFacturation)">
                                      <v-list-item-icon>
                                        <v-icon color="#B72C2C">print</v-icon>
                                      </v-list-item-icon>
                                      <v-list-item-title style="margin-left: -20px">Facture Synthèse des Privé(e)s
                                    ...
                                    <v-list-item v-if="item.categoriemaladiemvt == 'ABONNE(E)'" link
                                      @click="printFactureAbonneeGF_Synthese(item.refEnteteFacturation)">
                                      <v-list-item-icon>
                                        <v-icon color="#B72C2C">print</v-icon>
                                      </v-list-item-icon>
                                      <v-list-item-title style="margin-left: -20px">Facture Synthèse des Abonné(e)s
                                        (A4)</v-list-item-title>
                                    </v-list-item>

                                    <v-list-item v-if="item.categoriemaladiemvt == 'ABONNE(E)'" link
                                      @click="printFactureAbonneePF_Synthese(item.refEnteteFacturation)">
                                      <v-list-item-icon>
                                        <v-icon color="#B72C2C">print</v-icon>
                                      </v-list-item-icon>
                                      <v-list-item-title style="margin-left: -20px">Facture Synthèse des Abonné(e)s
...
export default {
  ...
  methods: {
    ...
    printInvoice() {
      this.svData.refEnteteFacturation = this.refEnteteFacturation;
      this.printFacturePriveePF(this.svData.refEnteteFacturation)
    },
    showCreatePaiement() {
      this.dialog4 = true;
    },
    validate_paie() {
      ...
      this.svData.author = this.userData.name;
      this.insertOrUpdate(
        `${this.apiBaseURL}/insert_paiefacturation_direct`,
        JSON.stringify(this.svData)
      )
        .then(({ data }) => {
          this.showMsg(data.data);
      ...
      this.editOrFetch(`${this.apiBaseURL}/fetch_single_detailfacturation/${id}`).then(
        ({ data }) => {
          ...
          donnees.map((item) => {
            ...
            this.svData.prixunitaire = item.prixunitaire;
    ...
    printFactureAbonneePF(id) {
      window.open(`${this.apiBaseURL}/pdf_petite_facture_abonnee_data?id=` + id);
    },
    ...
    showDataMedcin(id_data) {
      ...
      this.showDataBesoinService(id_data);
    ...
    showDataBesoinService(id) {
      this.editOrFetch(`${this.apiBaseURL}/fetch_besoinservice_episode/${id}`).then(
        ({ data }) => {
          var donnees = data.data;
          donnees.map((item) => {
            this.examens =  this.examens + " ; Etat de besoin des Infirmiers : "+ "  --  " + item.nom_medicament;
          });
          // this.dialog5 = true;
        }
        //
