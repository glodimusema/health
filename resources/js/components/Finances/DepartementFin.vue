            ...
            <v-form ref="form" lazy-validation>
              ...
              <v-card-text>
                <v-text-field label="Designation" prepend-inner-icon="extension" :rules="[(v) => !!v || 'Ce champ est requis']" outlined
                  v-model="svData.nom_departement"></v-text-field>

                  <v-text-field label="Code Departement" prepend-inner-icon="extension" :rules="[(v) => !!v || 'Ce champ est requis']" outlined
                  v-model="svData.code_departement"></v-text-field>
              </v-card-text>
              <v-card-actions>
                <v-spacer></v-spacer>
...
export default {
  ...
  methods: {
    ...
    showModal() {
      this.dialog = true;
      this.titleComponent = "Ajout Departement ";
      this.edit = false;
    ...
    testTitle() {
      if (this.edit == true) {
        this.titleComponent = "modification de " + item.nom_departement;
      } else {
  ...
  //   }, 300),
    onPageChange() {
      this.fetch_data(`${this.apiBaseURL}/fetch_departementfin?page=`);
    },
    ...
    validate() {
      ...
      if (this.$refs.form.validate()) {
        this.isLoading(true);          
        this.insertOrUpdate(
          `${this.apiBaseURL}/insert_departementfin`,
    ...
    editData(id) {
      this.editOrFetch(`${this.apiBaseURL}/fetch_single_departementfin/${id}`).then(
        ({ data }) => {
          var donnees = data.data;
    ...
    clearP(id) {
      this.confirmMsg().then(({ res }) => {
        this.delGlobal(`${this.apiBaseURL}/delete_departementfin/${id}`).then(
          ({ data }) => {
