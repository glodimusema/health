<template>
              ...
              <v-form ref="form" lazy-validation>
                ...
                <v-card-text>
                  <v-text-field
                    ...
                    outlined
                    v-model="svData.nom_examencolore"
                  ></v-text-field>
                </v-card-text>
                <v-card-actions>
                  <v-spacer></v-spacer>
  ...
  export default {
    ...
    methods: {
      ...
      showModal() {
        this.dialog = true;
        this.titleComponent = "Ajout Examen Coloré";
        this.edit = false;
      ...
      testTitle() {
        if (this.edit == true) {
          this.titleComponent = "modification de " + item.nom_examencolore;
        } else {
    ...
    //   }, 300),
      onPageChange() {
        this.fetch_data(`${this.apiBaseURL}/fetch_examencolore_labo?page=`);
      },
      fetchAccess() {
      ...
      validate() {
        if (this.$refs.form.validate()) {
          ...
          this.insertOrUpdate(
            `${this.apiBaseURL}/insert_examencolore_labo`,
      ...
      editData(id) {
        this.editOrFetch(`${this.apiBaseURL}/fetch_single_examencolore_labo/${id}`).then(
          ({ data }) => {
            var donnees = data.data;
      ...
      clearP(id) {
        this.confirmMsg().then(({ res }) => {
          this.delGlobal(`${this.apiBaseURL}/delete_examencolore_labo/${id}`).then(
            ({ data }) => {
