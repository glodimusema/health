<template>
  ...
  export default {
    ...
    methods: {
      ...
      showModal() {
        this.dialog = true;
        this.titleComponent = "Ajout Catégorie Valeur ";
        this.edit = false;
      ...
      testTitle() {
        if (this.edit == true) {
          ...
          this.titleComponent = "Ajout CatégorieValeur";
        }
      }
      ,
    ...
    //   }, 300),
      onPageChange() {
        this.fetch_data(`${this.apiBaseURL}/fetch_categorievaleur?page=`);
      },
      fetchAccess() {
      ...
      validate() {
        if (this.$refs.form.validate()) {
          ...
          this.insertOrUpdate(
            `${this.apiBaseURL}/insert_categorievaleur`,
      ...
      editData(id) {
        this.editOrFetch(`${this.apiBaseURL}/fetch_single_categorievaleur/${id}`).then(
          ({ data }) => {
            var donnees = data.data;
      ...
      clearP(id) {
        this.confirmMsg().then(({ res }) => {
          this.delGlobal(`${this.apiBaseURL}/delete_categorievaleur/${id}`).then(
            ({ data }) => {
