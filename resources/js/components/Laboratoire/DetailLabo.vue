...
<v-row justify="center">
                      ...
                      <v-flex xs12 sm12 md12 lg12>
                          ...
                          <v-flex xs12 sm12 md12 lg12>
                              <div class="mr-1">
                                ...
                                <v-text-field label="Les Echantillons" prepend-inner-icon="extension" dense :rules="[(v) => !!v || 'Ce champ est requis']" outlined
                                 v-model="svData.natureechantillon"></v-text-field>
                  ...
                  <v-layout>
                    ...
                    <v-flex md1>
                      <v-tooltip bottom color="black">
                        <template v-slot:activator="{ on, attrs }">
                          <span v-bind="attrs" v-on="on">
                            <v-btn @click="createResultat()" fab color="#B72C2C" dark>
                              <v-icon>add</v-icon>
                            </v-btn>
                          </span>
                        </template>
                        ...
                        <template v-slot:default>
                          <thead>
                            <tr>
                              ...
                              <th class="text-left">ValeurNormal</th>  
                          ...
                          <tbody>
                            <tr v-for="item in fetchData" :key="item.id">
                              ...
                              <template v-slot:activator="{ on, attrs }">
                                <span v-bind="attrs" v-on="on">
                                  <v-btn @click="printBill(item.refEnteteLabo)" fab small
                              ...
                              <td>{{ item.libelle }} - {{ item.commentaire }}</td>
  ...
  //,'natureechantillon','methode','commentaire'
    ...
    methods: {
      ...
      createResultat()
      {
        this.getDataEchantillon(this.refEntetePrelevement); 
        this.dialog = true; 
      }
      ,
      ...
      validate() {
        if (this.$refs.form.validate()) {
          ...
          if (this.edit) {
            ...
            this.insertOrUpdate(
              `${this.apiBaseURL}/update_detaillaboratoire/${this.svData.id}`,
     ...
     editData(id) {
        this.editOrFetch(`${this.apiBaseURL}/fetch_single_detaillaboratoire/${id}`).then(
          ({ data }) => {
            var donnees = data.data;
            donnees.map((item) => {  
      ...
      deleteData(id) {
        this.confirmMsg().then(({ res }) => {
          this.delGlobal(`${this.apiBaseURL}/delete_detaillaboratoire/${id}`).then(
            ({ data }) => {
              this.showMsg(data.data);
              this.fetchDataList();
            }
      ...
      fetchDataList() {
        this.fetch_data(`${this.apiBaseURL}/fetch_detail_entetelaboratoire/${this.refEnteteLabo}?page=`);
        this.showData(this.refEnteteLabo); 
        
      },  
      getDataEchantillon(refEntetePrelevement) {
      this.svData.natureechantillon = "";
      this.editOrFetch(`${this.apiBaseURL}/fetch_echantillon_prelevement/${refEntetePrelevement}`).then(
        ({ data }) => {
          var donnees = data.data;
          donnees.map((item) => {
            this.svData.natureechantillon = this.svData.natureechantillon + "  -  " + item.designation;
          });
          // this.dialog5 = true;
        }
        //
