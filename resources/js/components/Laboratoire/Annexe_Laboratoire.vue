                      ...
                      <v-flex xs12 sm12 md12 lg12>
                        <div class="mr-1">
                          <v-textarea label="Description du Document" prepend-inner-icon="draw" dense
                            :rules="[(v) => !!v || 'Ce champ est requis']" outlined v-model="svData.descriptionImage">
                          </v-textarea>
                        </div>
                      </v-flex>                    
                              ...
                              <td>
                                ...
                                <v-menu bottom rounded offset-y transition="scale-transition">
                                  ...
                                  <v-list dense width="">
                                    ...
                                    <v-list-item link @click="printBill(item.pdfLabo)">
                                    <v-list-item-icon>
  ...
  methods: {
    ...
    updatePhoto() {
      ...
      if (this.edit == true) {
        ...
        axios
          .post(`${this.apiBaseURL}/update_annexe_laboratoire`, formData, config)
          .then(({ data }) => {
            this.image = "";
        ...
        this.svData.author = this.userData.name;
        axios
          .post(`${this.apiBaseURL}/insert_annexe_laboratoire`, formData, config)
          .then(({ data }) => {
            this.image = "";
            this.showMsg(data.data);
    ...
    printBill(filenamess) {
      window.open(`${this.apiBaseURL}/downloadfile_labo/${filenamess}`);
    },

    editData(id) {
      this.editOrFetch(`${this.apiBaseURL}/fetch_single_annexe_laboratoire/${id}`).then(
        ({ data }) => {
          var donnees = data.data;
          donnees.map((item) => {
            this.svData.id = item.id;
              this.svData.refEnteteLabo = item.refEnteteLabo;
              this.svData.descriptionImage = item.descriptionImage;          
    ...
    deleteData(id) {
      this.confirmMsg().then(({ res }) => {
        this.delGlobal(`${this.apiBaseURL}/delete_annexe_laboratoire/${id}`).then(
          ({ data }) => {
            this.showMsg(data.data);
            this.fetchDataList();
          }
    ...
    fetchDataList() {
           this.fetch_data(`${this.apiBaseURL}/fetch_annexe_laboratoire_labo/${this.refEnteteLabo}?page=`);
      //
    }
