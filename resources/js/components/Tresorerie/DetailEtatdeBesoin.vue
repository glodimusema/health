              ...
              <v-dialog v-model="dialog" max-width="600px" persistent>
                <v-card :loading="loading">
                  <v-form ref="form" lazy-validation>
                    <v-card-title>
                      Details Etat de Besoin <v-spacer></v-spacer>
                    ...
                    <v-card-text>
                      ...
                      <v-select label="Selectionnez le Service Bénéficiaire" prepend-inner-icon="mdi-map"
                        :rules="[(v) => !!v || 'Ce champ est requis']" :items="ServicesList" item-text="nomProvenance" item-value="nomProvenance"
                        outlined dense v-model="svData.service_beneficiaire">
                      </v-select>
...
export default {
  ...
  methods: {
    ...
    editData(id) {
      this.editOrFetch(`${this.apiBaseURL}/fetch_single_DetatBesoin/${id}`).then(
        ({ data }) => {
          var donnees = data.data;
          donnees.map((item) => {
    ...
    deleteData(id) {
      this.confirmMsg().then(({ res }) => {
        this.delGlobal(`${this.apiBaseURL}/delete_DetatBesoin/${id}`).then(
          ({ data }) => {
            this.showMsg(data.data);
            this.fetchDataList();
          }
    ...
    fetchDataList() {      
      this.fetch_data(`${this.apiBaseURL}/fetch_detail_enteteetatbesoin/${this.refEntete}?page=`);
