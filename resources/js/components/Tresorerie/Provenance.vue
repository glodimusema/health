            ...
            <v-form ref="form" lazy-validation>
              ...
              <v-card-text>
                <v-text-field label="Designation" prepend-inner-icon="extension" dense
                  :rules="[(v) => !!v || 'Ce champ est requis']" outlined v-model="svData.nomProvenance"></v-text-field>

                  <v-text-field label="Code Service" prepend-inner-icon="extension" dense
                  :rules="[(v) => !!v || 'Ce champ est requis']" outlined v-model="svData.codeProvenance"></v-text-field>
              </v-card-text>
              <v-card-actions>
                <v-spacer></v-spacer>
              ...
              <template v-slot:default>
                <thead>
                  <tr>
                    ...
                    <th class="text-left">CodeService</th>
...
export default {
  ...
  methods: {
    ...
    showModal() {
      this.dialog = true;
      this.titleComponent = "Ajout Services ";
      this.edit = false;
    ...
    testTitle() {
      if (this.edit == true) {
        this.titleComponent = "modification de " + item.nomProvenance;
      } else {
    ...
    ,
    onPageChange() {
      this.fetch_data(`${this.apiBaseURL}/fetch_all_provenance?page=`);
    },
      fetchAccess() {
    ...
    validate() {
      if (this.$refs.form.validate()) {
        ...
        this.insertOrUpdate(
          `${this.apiBaseURL}/insert_provenance`,
    ...
    editData(id) {
      this.editOrFetch(`${this.apiBaseURL}/fetch_single_provenance/${id}`).then(
        ({ data }) => {
          var donnees = data.data;
    ...
    clearP(id) {
      this.confirmMsg().then(({ res }) => {
        this.delGlobal(`${this.apiBaseURL}/delete_provenance/${id}`).then(
          ({ data }) => {
