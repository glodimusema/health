<template>
  ...
  export default {
    ...
    methods: {
      ...
      showModal() {
        this.dialog = true;
        this.titleComponent = "Ajout Libellé ";
        this.edit = false;
      ...
      testTitle() {
        if (this.edit == true) {
          this.titleComponent = "modification de " + item.desi_libelle;
        } else {
    ...
    //   }, 300),
      onPageChange() {
        this.fetch_data(`${this.apiBaseURL}/fetch_all_libelle_score?page=`);
      },
      ...
      validate() {
        if (this.$refs.form.validate()) {
          ...
          this.insertOrUpdate(
            `${this.apiBaseURL}/insert_libelle_score`,
      ...
      editData(id) {
        this.editOrFetch(`${this.apiBaseURL}/fetch_single_libelle_score/${id}`).then(
          ({ data }) => {
      ...
      clearP(id) {
        this.confirmMsg().then(({ res }) => {
          this.delGlobal(`${this.apiBaseURL}/delete_libelle_score/${id}`).then(
            ({ data }) => {
