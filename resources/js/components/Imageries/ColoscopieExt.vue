                  ...
                  <v-form ref="form" lazy-validation>
                    ...
                    <v-card-text>
                      ...
                      <v-flex xs12 sm12 md12 lg12>
                        <div class="mr-1">
                          <v-textarea label="Inspection" prepend-inner-icon="draw" dense
                            :rules="[(v) => !!v || 'Ce champ est requis']" outlined v-model="svData.inspection">
                          </v-textarea>
                        </div>
                      </v-flex>
                      ...
                      <v-flex xs12 sm12 md12 lg12>
                        <div class="mr-1">
                          <v-textarea label="Touché Réctal" prepend-inner-icon="draw" dense
                            :rules="[(v) => !!v || 'Ce champ est requis']" outlined v-model="svData.toucherRectal">
                          </v-textarea>
                        </div>
                      </v-flex>
                      ...
                      <v-flex xs12 sm12 md12 lg12>
                        <div class="mr-1">
                          <v-textarea label="Anuscopie" prepend-inner-icon="draw" dense
                            :rules="[(v) => !!v || 'Ce champ est requis']" outlined v-model="svData.anuscopie">
                          </v-textarea>
                        </div>
                      </v-flex> 
                      ...
                      <v-flex xs12 sm12 md12 lg12>
                        <div class="mr-1">
                          <v-textarea label="Coloscopie" prepend-inner-icon="draw" dense
                            :rules="[(v) => !!v || 'Ce champ est requis']" outlined v-model="svData.coloscopie">
                          </v-textarea>
                        </div>
                      </v-flex> 
                      ...
                      <v-flex xs12 sm12 md12 lg12>
                        <div class="mr-1">
                          <v-textarea label="Biopsies" prepend-inner-icon="draw" dense
                            :rules="[(v) => !!v || 'Ce champ est requis']" outlined v-model="svData.biopsies">
                          </v-textarea>
                        </div>
                      </v-flex> 
...
export default {
  ...
  data() {
    return {
      ...
      //'id',refImagerie, inspection, toucherRectal, anuscopie, coloscopie, biopsies, conclusion, image_colorscopie, author
      svData: {
        id: '',
        refImagerie: 0,        
        inspection: "",
  ...
  methods: {
    ...
    updatePhoto() {
      ...
      if (this.edit == true) {
        ...
        axios
          .post(`${this.apiBaseURL}/update_coloscopi_ext`, formData, config)
          .then(({ data }) => {
            this.image = "";
        ...
        this.svData.author = this.userData.name;
        axios
          .post(`${this.apiBaseURL}/insert_coloscopi_ext`, formData, config)
          .then(({ data }) => {
            this.image = "";
            this.showMsg(data.data);
    ...
    editData(id) {
      this.editOrFetch(`${this.apiBaseURL}/fetch_single_coloscopi_ext/${id}`).then(
        ({ data }) => {
          var donnees = data.data;
          donnees.map((item) => {
            this.svData.id = item.id;
              this.svData.refImagerie = item.refImagerie;
              this.svData.inspection = item.inspection;
              this.toucherRectal= item.toucherRectal,
              this.anuscopie= item.anuscopie,
              this.coloscopie= item.coloscopie,
              this.biopsies= item.biopsies,
    ...
    deleteData(id) {
      this.confirmMsg().then(({ res }) => {
        this.delGlobal(`${this.apiBaseURL}/delete_coloscopi_ext/${id}`).then(
          ({ data }) => {
            this.showMsg(data.data);
            this.fetchDataList();
          }
    ...
    fetchDataList() {
           this.fetch_data(`${this.apiBaseURL}/fetch_coloscopie_imagerie_ext/${this.refImagerie}?page=`);
      //
    }
