    ...
    validate() {
      if (this.$refs.form.validate()) {
        ...
        if (this.edit) {
          ...
          this.insertOrUpdate(
            `${this.apiBaseURL}/update_resultat_ECG_ext/${this.svData.id}`,
          ...
          this.svData.author = this.userData.name;
          this.insertOrUpdate(
            `${this.apiBaseURL}/insert_resultat_ECG_ext`,
            JSON.stringify(this.svData)
          )
            .then(({ data }) => {
              this.showMsg(data.data);
...
getSpecialiteMedecin2(idMedecin) {
  ...
  getSpecialiteMedecin4(idMedecin) {
    ...
    editData(id) {
      this.editOrFetch(`${this.apiBaseURL}/fetch_single_resultat_ECG_ext/${id}`).then(
        ({ data }) => {
          var donnees = data.data;
          donnees.map((item) => {
    ...
    deleteData(id) {
      this.confirmMsg().then(({ res }) => {
        this.delGlobal(`${this.apiBaseURL}/delete_resultat_ECG_ext/${id}`).then(
          ({ data }) => {
            this.showMsg(data.data);
            this.fetchDataList();
          }
    ...
    fetchDataList() {
      this.fetch_data(`${this.apiBaseURL}/fetch_resultat_imagerie_ECG_ext/${this.refImagerie}?page=`);
      //
    }
