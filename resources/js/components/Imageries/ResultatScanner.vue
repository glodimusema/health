                ...
                <v-dialog v-model="dialog" max-width="900px" persistent>
                  <v-card :loading="loading">
                    <v-form ref="form" lazy-validation>
                      <v-card-title>
                        Resultat Scanneer <v-spacer></v-spacer>
                        ...
                        <v-layout row wrap>
                          ...
                          <div class="mr-1">
                            <v-textarea label="Technique/Indication" prepend-inner-icon="draw" dense
                              :rules="[(v) => !!v || 'Ce champ est requis']" outlined v-model="svData.technique_res">
                            </v-textarea>
                          </div>
                        </v-flex>
                        ...
                        <v-flex xs12 sm12 md12 lg12>
                          <div class="mr-1">
                            <v-textarea label="Description" prepend-inner-icon="draw" dense
                              :rules="[(v) => !!v || 'Ce champ est requis']" outlined v-model="svData.description_res">
                            </v-textarea>
                          </div>
                        </v-flex>
                        ...
                        <v-flex xs12 sm12 md12 lg12>
                          <div class="mr-1">
                            <v-textarea label="En conclusion" prepend-inner-icon="draw" dense
                              :rules="[(v) => !!v || 'Ce champ est requis']" outlined v-model="svData.conclusion_res">
                            </v-textarea>
                          </div>
                        </v-flex>
                                  ...
                                  <v-menu bottom rounded offset-y transition="scale-transition">
                                    ...
                                    <v-list dense width="">
                                      ...
                                      <v-list-item link @click="printBill2(item.id)">
                                        ...
                                        <v-list-item-title style="margin-left: -20px">Voir Resultat(Image)</v-list-item-title>
                                      </v-list-item>
  ...
  methods: {
    ...
    updatePhoto() {
      ...
      if (this.edit == true) {
        ...
        axios
          .post(`${this.apiBaseURL}/update_ResultatImage`, formData, config)
          .then(({ data }) => {
            this.image = "";
        ...
        this.svData.author = this.userData.name;
        axios
          .post(`${this.apiBaseURL}/insert_ResultatImage`, formData, config)
          .then(({ data }) => {
            this.image = "";
            this.showMsg(data.data);
    ...
    printBill(id) {
      window.open(`${this.apiBaseURL}/pdf_protocoleimagerie_data?id=` + id);
    },

    printBill2(id) {
      window.open(`${this.apiBaseURL}/pdf_protocoleimagerie_image_data?id=` + id);
    },

    editData(id) {
      this.editOrFetch(`${this.apiBaseURL}/fetch_single_ResultatImage/${id}`).then(
        ({ data }) => {
          var donnees = data.data;
          donnees.map((item) => {
            this.svData.id = item.id;
              this.svData.refImagerie = item.refImagerie;              
              this.svData.technique_res = item.technique_res;
              this.svData.description_res = item.description_res;
              this.svData.conclusion_res = item.conclusion_res;             
    ...
    deleteData(id) {
      this.confirmMsg().then(({ res }) => {
        this.delGlobal(`${this.apiBaseURL}/delete_ResultatImage/${id}`).then(
          ({ data }) => {
            this.showMsg(data.data);
            this.fetchDataList();
          }
    ...
    fetchDataList() {
      this.fetch_data(`${this.apiBaseURL}/fetch_resultat_imagerie/${this.refImagerie}?page=`);
    }
