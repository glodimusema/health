                  ...
                  <v-form ref="form" lazy-validation>
                    ...
                    <v-card-text>
                      ...
                      <v-layout row wrap>
                        ...
                        <div class="mr-1">
                          <v-textarea label="Indication" prepend-inner-icon="draw" dense
                            :rules="[(v) => !!v || 'Ce champ est requis']" outlined v-model="svData.indication">
                          </v-textarea>
                        </div>
                      </v-flex>
                      ...
                      <v-flex xs12 sm12 md12 lg12>
                        <div class="mr-1">
                          <v-textarea label="Ventricule Gauche" prepend-inner-icon="draw" dense
                            :rules="[(v) => !!v || 'Ce champ est requis']" outlined v-model="svData.ventriculeGauche">
                          </v-textarea>
                        </div>
                      </v-flex>
                      ...
                      <v-flex xs12 sm12 md12 lg12>
                        <div class="mr-1">
                          <v-textarea label="Ventricule Droite" prepend-inner-icon="draw" dense
                            :rules="[(v) => !!v || 'Ce champ est requis']" outlined v-model="svData.ventriculeDroite">
                          </v-textarea>
                        </div>
                      </v-flex>
                      ...
                      <v-flex xs12 sm12 md12 lg12>
                        <div class="mr-1">
                          <v-textarea label="Oreilletes" prepend-inner-icon="draw" dense
                            :rules="[(v) => !!v || 'Ce champ est requis']" outlined v-model="svData.oreillette">
                          </v-textarea>
                        </div>
                      </v-flex>
                      ...
                      <v-flex xs12 sm12 md12 lg12>
                        <div class="mr-1">
                          <v-textarea label="Valves" prepend-inner-icon="draw" dense
                            :rules="[(v) => !!v || 'Ce champ est requis']" outlined v-model="svData.valve">
                          </v-textarea>
                        </div>
                      </v-flex>
                      ...
                      <v-flex xs12 sm12 md12 lg12>
                        <div class="mr-1">
                          <v-textarea label="Gros vaisseaux" prepend-inner-icon="draw" dense
                            :rules="[(v) => !!v || 'Ce champ est requis']" outlined v-model="svData.oesophage">
                          </v-textarea>
                        </div>
                      </v-flex>
                      ...
                      <v-flex xs12 sm12 md12 lg12>
                        <div class="mr-1">
                          <v-textarea label="En conclusion" prepend-inner-icon="draw" dense
                            :rules="[(v) => !!v || 'Ce champ est requis']" outlined v-model="svData.conclusionCardio">
                          </v-textarea>
                        </div>
                      </v-flex>
  ...
  methods: {
    ...
    updatePhoto() {
      ...
      if (this.edit == true) {
        ...
        axios
          .post(`${this.apiBaseURL}/update_Cardiologie_ext`, formData, config)
          .then(({ data }) => {
            this.image = "";
        ...
        this.svData.author = this.userData.name;
        axios
          .post(`${this.apiBaseURL}/insert_Cardiologie_ext`, formData, config)
          .then(({ data }) => {
            this.image = "";
            this.showMsg(data.data);
    ...
    editData(id) {
      this.editOrFetch(`${this.apiBaseURL}/fetch_single_CardiologieImage_ext/${id}`).then(
        ({ data }) => {
          var donnees = data.data;
          donnees.map((item) => {
            this.svData.id = item.id;
              ...
              this.svData.ventriculeGauche = item.ventriculeGauche;
              this.svData.ventriculeDroite = item.ventriculeDroite;
              this.svData.oreillette = item.oreillette;
              this.svData.valve = item.valve;
              this.svData.oesophage = item.oesophage;
              this.svData.autres = item.autres;
              this.svData.conclusionCardio = item.conclusionCardio;             
    ...
    deleteData(id) {
      this.confirmMsg().then(({ res }) => {
        this.delGlobal(`${this.apiBaseURL}/delete_Cardiologie_ext/${id}`).then(
          ({ data }) => {
            this.showMsg(data.data);
            this.fetchDataList();
          }
    ...
    fetchDataList() {
      this.fetch_data(`${this.apiBaseURL}/fetch_cardiologie_imagerie_ext/${this.refImagerie}?page=`);
      //
    }
