                ...
                <v-card :loading="loading">
                  <v-form ref="form" lazy-validation>
                    ...
                    <v-card-text>
                      ...
                      <v-flex xs12 sm12 md12 lg12>
                        <div class="mr-1">
                          <v-textarea label="Rectosigmoidoscopie" prepend-inner-icon="draw" dense
                            :rules="[(v) => !!v || 'Ce champ est requis']" outlined v-model="svData.rectosigmoidoscopie">
                          </v-textarea>
                        </div>
                      </v-flex> 
  ...
  methods: {
    ...
    updatePhoto() {
      ...
      if (this.edit == true) {
        ...
        axios
          .post(`${this.apiBaseURL}/update_rectosigmoidocopie_ext`, formData, config)
          .then(({ data }) => {
            this.image = "";
        ...
        this.svData.author = this.userData.name;
        axios
          .post(`${this.apiBaseURL}/insert_rectosigmoidocopie_ext`, formData, config)
          .then(({ data }) => {
            this.image = "";
            this.showMsg(data.data);
    ...
    editData(id) {
      this.editOrFetch(`${this.apiBaseURL}/fetch_single_rectosigmoidocopie_ext/${id}`).then(
        ({ data }) => {
          var donnees = data.data;
          donnees.map((item) => {
            this.svData.id = item.id;
              ...
              this.rectosigmoidoscopie= item.rectosigmoidoscopie,
    ...
    deleteData(id) {
      this.confirmMsg().then(({ res }) => {
        this.delGlobal(`${this.apiBaseURL}/delete_rectosigmoidocopie_ext/${id}`).then(
          ({ data }) => {
            this.showMsg(data.data);
            this.fetchDataList();
          }
    ...
    fetchDataList() {
           this.fetch_data(`${this.apiBaseURL}/fetch_rectosigmoidocopie_imagerie_ext/${this.refImagerie}?page=`);
      //
    }
