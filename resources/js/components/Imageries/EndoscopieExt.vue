                      ...
                      <v-flex xs12 sm12 md12 lg12>
                        <div class="mr-1">
                          <v-textarea label="Description" prepend-inner-icon="draw" dense
                            :rules="[(v) => !!v || 'Ce champ est requis']" outlined v-model="svData.descriptionEndo">
                          </v-textarea>
                        </div>
                      </v-flex>
                      ...
                      <v-flex xs12 sm12 md12 lg12>
                        <div class="mr-1">
                          <v-textarea label="En conclusion" prepend-inner-icon="draw" dense
                            :rules="[(v) => !!v || 'Ce champ est requis']" outlined v-model="svData.conclusionEndo">
                          </v-textarea>
                        </div>
                      </v-flex>
...
export default {
  ...
  data() {
    return {
      ...
      //'id','refImagerie','descriptionEndo','conclusionEndo','imageEndo','author'
      svData: {
        id: '',
        refImagerie: 0,        
        descriptionEndo: "",
  ...
  methods: {
    ...
    updatePhoto() {
      ...
      if (this.edit == true) {
        ...
        axios
          .post(`${this.apiBaseURL}/update_Endoscopie_ext`, formData, config)
          .then(({ data }) => {
            this.image = "";
        ...
        this.svData.author = this.userData.name;
        axios
          .post(`${this.apiBaseURL}/insert_Endoscopie_ext`, formData, config)
          .then(({ data }) => {
            this.image = "";
            this.showMsg(data.data);
    ...
    editData(id) {
      this.editOrFetch(`${this.apiBaseURL}/fetch_single_EndoscopieImage_ext/${id}`).then(
        ({ data }) => {
          var donnees = data.data;
          donnees.map((item) => {
            this.svData.id = item.id;
              this.svData.refImagerie = item.refImagerie;
              this.svData.descriptionEndo = item.descriptionEndo;
              this.svData.conclusionEndo = item.conclusionEndo;             
    ...
    deleteData(id) {
      this.confirmMsg().then(({ res }) => {
        this.delGlobal(`${this.apiBaseURL}/delete_Endoscopie_ext/${id}`).then(
          ({ data }) => {
            this.showMsg(data.data);
            this.fetchDataList();
          }
    ...
    fetchDataList() {
           this.fetch_data(`${this.apiBaseURL}/fetch_endoscopie_imagerie_ext/${this.refImagerie}?page=`);
      //
    }
