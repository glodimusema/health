                        ...
                        <v-flex xs12 sm12 md12 lg12>
                          <div class="mr-1">
                            <v-textarea label="Description" prepend-inner-icon="draw" dense
                              :rules="[(v) => !!v || 'Ce champ est requis']" outlined v-model="svData.descriptionAttest">
                            </v-textarea>
                          </div>
                        </v-flex>
                        ...
                        <v-flex xs12 sm12 md12 lg12>
                          <div class="mr-1">
                            <v-textarea label="Conclusion"
                              ...
                              v-model="svData.conclusionAttest">
                            </v-textarea>
                          </div>
                        </v-flex>
...
export default {
  ...
  data() {
    return {
      ...
      //id,refDetailConst,descriptionAttest,conclusionAttestAttest,medecin1,specialite1,cnom1,medecin2,specialite2,cnom2,
    ...
    validate() {
      if (this.$refs.form.validate()) {
        ...
        if (this.edit) {
          ...
          this.insertOrUpdate(
            `${this.apiBaseURL}/update_imagerie_attestation/${this.svData.id}`,
          ...
          this.svData.author = this.userData.name;
          this.insertOrUpdate(
            `${this.apiBaseURL}/insert_imagerie_attestation`,
            JSON.stringify(this.svData)
          )
            .then(({ data }) => {
              this.showMsg(data.data);
...
getSpecialiteMedecin2(idMedecin) {
  ...
  getSpecialiteMedecin4(idMedecin) {
    ...
    editData(id) {
      this.editOrFetch(`${this.apiBaseURL}/fetch_imagerie_single_attestation/${id}`).then(
        ({ data }) => {
          var donnees = data.data;
          donnees.map((item) => {
            this.svData.id = item.id;
            this.svData.refDetailConst = item.refDetailConst;
            this.svData.descriptionAttest= item.descriptionAttest;
            this.svData.conclusionAttest= item.conclusionAttest;
    ...
    deleteData(id) {
      this.confirmMsg().then(({ res }) => {
        this.delGlobal(`${this.apiBaseURL}/delete_imagerie_attestation/${id}`).then(
          ({ data }) => {
            this.showMsg(data.data);
            this.fetchDataList();
          }
    ...
    fetchDataList() {
      this.fetch_data(`${this.apiBaseURL}/fetch_imagerie_attestation_cons/${this.refDetailConst}?page=`);
      //
    }
