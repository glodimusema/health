                ...
                <v-card :loading="loading">
                  <v-form ref="form" lazy-validation>
                    ...
                    <v-card-text>
                      ...
                      <v-flex xs12 sm12 md12 lg12>
                        <div class="mr-1">
                          <v-textarea label="Rectoscopie" prepend-inner-icon="draw" dense
                            :rules="[(v) => !!v || 'Ce champ est requis']" outlined v-model="svData.rectoscopie">
                          </v-textarea>
                        </div>
                      </v-flex>                      
                      ...
                      <v-flex xs12 sm12 md12 lg12>
                        <div class="mr-1">
                          <v-textarea label="En Conclusion" prepend-inner-icon="draw" dense
                            :rules="[(v) => !!v || 'Ce champ est requis']" outlined v-model="svData.conclusion_recto">
                          </v-textarea>
                        </div>
                      </v-flex>
...
export default {
  ...
  data() {
    return {
      ...
      //'id',refImagerie,inspection,toucherRectal,anuscopie,image_rectoscopie,author
      svData: {
        id: '',
        refImagerie: 0,        
        inspection: "",
  ...
  methods: {
    ...
    updatePhoto() {
      ...
      if (this.edit == true) {
        ...
        axios
          .post(`${this.apiBaseURL}/update_rectoscopie_ext`, formData, config)
          .then(({ data }) => {
            this.image = "";
        ...
        this.svData.author = this.userData.name;
        axios
          .post(`${this.apiBaseURL}/insert_rectoscopie_ext`, formData, config)
          .then(({ data }) => {
            this.image = "";
            this.showMsg(data.data);
    ...
    editData(id) {
      this.editOrFetch(`${this.apiBaseURL}/fetch_single_rectoscopie_ext/${id}`).then(
        ({ data }) => {
          var donnees = data.data;
          donnees.map((item) => {
            this.svData.id = item.id;
              ...
              this.rectoscopie= item.rectoscopie,
              this.svData.conclusion_recto = item.conclusion_recto;             
    ...
    deleteData(id) {
      this.confirmMsg().then(({ res }) => {
        this.delGlobal(`${this.apiBaseURL}/delete_rectoscopie_ext/${id}`).then(
          ({ data }) => {
            this.showMsg(data.data);
            this.fetchDataList();
          }
    ...
    fetchDataList() {
           this.fetch_data(`${this.apiBaseURL}/fetch_rectoscopie_imagerie_ext/${this.refImagerie}?page=`);
      //
    }
