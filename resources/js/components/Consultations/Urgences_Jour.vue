    ...
    <v-flex md12>
      ...
      <v-dialog v-model="dialog3" max-width="400px" persistent>
        <v-card :loading="loading">
          <v-form ref="form" lazy-validation>
            <v-card-title>
              Cloturer l'Ugence <v-spacer></v-spacer>
              <v-tooltip bottom color="black">
                <template v-slot:activator="{ on, attrs }">
                  <span v-bind="attrs" v-on="on">
            ...
            <v-card-text>
              ...
              <v-flex xs12 sm12 md12 lg12>
                <div class="mr-1">
                  <v-select label="Voulez-vous Cloturer cette Urgence ?" :items="[
                    { designation: 'OUI' },
                    { designation: 'NON' }
                  ]" prepend-inner-icon="extension" :rules="[(v) => !!v || 'Ce champ est requis']" outlined dense
                    item-text="designation" item-value="designation" v-model="svData.finUrgence"></v-select>
                </div>
              </v-flex>
              <!--  -->
            </v-card-text>
      ...
      <v-layout row wrap>
        <v-flex xs12 sm12 md6 lg6>
          <div class="mr-1">
            <router-link :to="'/admin/urgences'">Urgenses/Urgences</router-link>
          </div>
        </v-flex>
      </v-layout>
                ...
                <template v-slot:default>
                  <thead>
                    <tr>
                      ...
                      <th class="text-left">N°Lit</th>
                      <th class="text-left">FinUrgence</th>
                      ...
                      <td>
                        ...
                        <v-menu bottom rounded offset-y transition="scale-transition">
                          ...
                          <v-list dense width="">
                            ...
                            <v-list-item link v-if="(item.duree <= item.nmbreJourConsMvt)"
                            ...
                            <v-list-item link @click="editDataCloture(item.id)">
                              <v-list-item-icon>
                                <v-icon color="#B72C2C">mdi-marker</v-icon>
                              </v-list-item-icon>
                              <v-list-item-title style="margin-left: -20px">Cloturer cette Urgence</v-list-item-title>
                            </v-list-item>
...
export default {
  ...
  methods: {
    ...
    fetchDataList() {
      ...
      this.fetch_data(`${this.apiBaseURL}/fetch_urgences_jour?page=`);
    },
      ...
      this.editOrFetch(`${this.apiBaseURL}/fetch_single_enteteconsultation/${id}`).then(
        ({ data }) => {
          ...
          donnees.map((item) => {
            ...
            this.svData.finUrgence = item.finUrgence;
          });
    ...
    validateCons() {
      if (this.$refs.form.validate()) {
        ...
        if (this.edit) {
          this.svData.author = this.userData.name;
          this.insertOrUpdate(
            `${this.apiBaseURL}/update_fin_urgence/${this.svData.id}`,
            JSON.stringify(this.svData)
          )//
            .then(({ data }) => {
              this.showMsg(data.data);
