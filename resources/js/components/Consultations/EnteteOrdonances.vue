              ...
              <v-dialog v-model="dialog" max-width="400px" persistent>
                <v-card :loading="loading">
                  <v-form ref="form" lazy-validation>
                    <v-card-title>
                      Entete Ordonance <v-spacer></v-spacer>
                    ...
                    <v-card-text>           
                        ...
                        <v-text-field type="date" label="Date Ordonance" prepend-inner-icon="extension" :rules="[(v) => !!v || 'Ce champ est requis']" outlined dense
                            v-model="svData.date_ordonance"></v-text-field>
                        ...
                        <template v-slot:default>
                          <thead>
                            <tr>
                              ...
                              <th class="text-left">DateOrdonance</th>                                                                                         
                          ...
                          <td>
                              ...
                              <v-list dense width="">
                                ...
                                <v-list-item link @click="showDetailOrdonance(item.id, item.noms)">
                                      <v-list-item-icon>
  ...
  export default {
    ...
    data() {
      return {
        ...
        titleComponent: '',
        refDetailCons: 0, 
        //id,refDetailCons,date_ordonance,author
        svData: {
          id: '',
          refDetailCons: 0,          
          date_ordonance:"",
    ...
    methods: {
      ...
      validate() {
        if (this.$refs.form.validate()) {
          ...
          if (this.edit) {
            ...
            this.insertOrUpdate(
              `${this.apiBaseURL}/update_entete_ordonance/${this.svData.id}`,
            ...
            this.svData.author = this.userData.name;
            this.insertOrUpdate(
              `${this.apiBaseURL}/insert_entete_ordonance`,
              JSON.stringify(this.svData)
            )
              .then(({ data }) => {
                this.showMsg(data.data);
      ...
      editData(id) {
        this.editOrFetch(`${this.apiBaseURL}/fetch_single_entete_ordonance/${id}`).then(
          ({ data }) => {
            var donnees = data.data;
            donnees.map((item) => {  
              this.svData.id = item.id;
              this.svData.refDetailCons = item.refDetailCons;
              this.svData.date_ordonance = item.date_ordonance;                          
            });
            this.edit = true;
            this.dialog = true;
           }
      ...
      deleteData(id) {
        this.confirmMsg().then(({ res }) => {
          this.delGlobal(`${this.apiBaseURL}/delete_entete_ordonance/${id}`).then(
            ({ data }) => {
              this.showMsg(data.data);
              this.fetchDataList();
            }
      ...
      fetchDataList() {
        this.fetch_data(`${this.apiBaseURL}/fetch_entete_ordonance_cons/${this.refDetailCons}?page=`);
        //
      },
