                  ...
                  <v-form ref="form" lazy-validation>
                    ...
                    <v-card-text>
                      ...
                      <v-flex xs12 sm12 md12 lg12>
                        <div class="mr-1">
                          <v-textarea label="Description du Document" prepend-inner-icon="draw" dense
                            :rules="[(v) => !!v || 'Ce champ est requis']" outlined v-model="svData.desicriptionPDF">
                          </v-textarea>
                        </div>
                      </v-flex>                    
                        ...
                        <template v-slot:default>
                          ...
                          <tbody>
                            <tr v-for="item in fetchData" :key="item.id">
                              ...
                              <td>{{ item.desicriptionPDF }}</td>
                              <td>{{ item.pdfPatient }}</td>
                              <td>
                                ...
                                <v-menu bottom rounded offset-y transition="scale-transition">
                                  ...
                                  <v-list dense width="">
                                    ...
                                    <v-list-item link @click="printBill(item.pdfPatient)">
                                    <v-list-item-icon>
...
export default {
  ...
  data() {
    return {
      ...
      //id,refPatient,pdfPatient,desicriptionPDF,author
      svData: {
        id: '',
        refPatient: 0,        
        desicriptionPDF: "",       
  ...
  methods: {
    ...
    updatePhoto() {
      ...
      if (this.edit == true) {
        ...
        axios
          .post(`${this.apiBaseURL}/update_annexe_patient`, formData, config)
          .then(({ data }) => {
            this.image = "";
        ...
        this.svData.author = this.userData.name;
        axios
          .post(`${this.apiBaseURL}/insert_annexe_patient`, formData, config)
          .then(({ data }) => {
            this.image = "";
            this.showMsg(data.data);
    ...
    printBill(filenamess) {
      window.open(`${this.apiBaseURL}/downloadfile_patient/${filenamess}`);
    },

    editData(id) {
      this.editOrFetch(`${this.apiBaseURL}/fetch_single_annexe_patient/${id}`).then(
        ({ data }) => {
          var donnees = data.data;
          donnees.map((item) => {
            this.svData.id = item.id;
              this.svData.refPatient = item.refPatient;
              this.svData.desicriptionPDF = item.desicriptionPDF;          
    ...
    deleteData(id) {
      this.confirmMsg().then(({ res }) => {
        this.delGlobal(`${this.apiBaseURL}/delete_annexe_patient/${id}`).then(
          ({ data }) => {
            this.showMsg(data.data);
            this.fetchDataList();
          }
    ...
    fetchDataList() {
           this.fetch_data(`${this.apiBaseURL}/fetch_annexe_patient/${this.refPatient}?page=`);
      //
    }
