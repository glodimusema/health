                      ...
                      <v-flex xs12 sm12 md12 lg12>
                        <div class="mr-1">
                          <v-textarea label="Description du Document" prepend-inner-icon="draw" dense
                            :rules="[(v) => !!v || 'Ce champ est requis']" outlined v-model="svData.descriptionPFD">
                          </v-textarea>
                        </div>
                      </v-flex>                    
                              ...
                              <td>
                                ...
                                <v-menu bottom rounded offset-y transition="scale-transition">
                                  ...
                                  <v-list dense width="">
                                    ...
                                    <v-list-item link @click="printBill(item.pdfNeuro)">
                                    <v-list-item-icon>
...
export default {
  ...
  methods: {
    ...
    updatePhoto() {
      ...
      if (this.edit == true) {
        this.svData.refProtocole = this.refProtocole;
        this.svData.author = this.userData.name;
        axios
          .post(`${this.apiBaseURL}/update_annexe_neurologie`, formData, config)
          .then(({ data }) => {
            this.image = "";
        ...
        this.svData.author = this.userData.name;
        axios
          .post(`${this.apiBaseURL}/insert_annexe_neurologie`, formData, config)
          .then(({ data }) => {
            this.image = "";
            this.showMsg(data.data);
    ...
    editData(id) {
      this.editOrFetch(`${this.apiBaseURL}/fetch_single_annexe_neurologie/${id}`).then(
        ({ data }) => {
          var donnees = data.data;
          donnees.map((item) => {
            this.svData.id = item.id;
              this.svData.refProtocole = item.refProtocole;
              this.svData.descriptionPFD = item.descriptionPFD;          
    ...
    deleteData(id) {
      this.confirmMsg().then(({ res }) => {
        this.delGlobal(`${this.apiBaseURL}/delete_annexe_neurologie/${id}`).then(
          ({ data }) => {
            this.showMsg(data.data);
            this.fetchDataList();
          }
    ...
    fetchDataList() {
           this.fetch_data(`${this.apiBaseURL}/fetch_annexe_neuro_protocole/${this.refProtocole}?page=`);
      //
    }
