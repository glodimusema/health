      ...
      <v-flex md12>
        <v-dialog v-model="dialog" max-width="400px" persistent>
          <v-card :loading="loading">
            <v-form ref="form" lazy-validation>
              <v-card-title>
                Maladie Chronique <v-spacer></v-spacer>
              ...
              <v-card-text>               
                ...
                <v-flex xs12 sm12 md12 lg12>
                    ...
                    <v-text-field readonly label="Catégorie" prepend-inner-icon="extension" :rules="[(v) => !!v || 'Ce champ est requis']" outlined dense
                      ...
                      <v-textarea type="textarea" label="Autres Details" prepend-inner-icon="draw" dense
                      :rules="[(v) => !!v || 'Ce champ est requis']" outlined v-model="svData.autredetail "></v-textarea>
        ...
        <v-flex xs12 sm12 md6 lg6>
          <div class="mr-1">
            ...
            <router-link :to="'/admin/MaladieChronique/'+this.svData.refMalade">/Signaler Maladies Chroniques</router-link>                                 
          </div>
        </v-flex>
        </v-layout>
                    ...
                    <tbody>
                      <tr v-for="item in fetchData" :key="item.id">
                        ...
                        <v-list dense width="">
                          ...
                          <!-- <v-list-item link  @click="printBill(item.refMalade)">
                            <v-list-item-icon>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      validate() {
        if (this.$refs.form.validate()) {
          ...
          if (this.edit) {
            ...
            this.insertOrUpdate(
              `${this.apiBaseURL}/update_chronique/${this.svData.id}`,
            ...
            this.svData.author = this.userData.name;
            this.insertOrUpdate(
              `${this.apiBaseURL}/insert_chronique`,
              JSON.stringify(this.svData)
            )
              .then(({ data }) => {
                this.showMsg(data.data);
      ...
      editData(id) {
        this.editOrFetch(`${this.apiBaseURL}/fetch_single_chronique/${id}`).then(
          ({ data }) => {
            var donnees = data.data;
            donnees.map((item) => {  
            ...
            getCategorie(this.svData.refmaladie);
            this.edit = true;
            this.dialog = true;
           }
        );
      ...
      deleteData(id) {
        this.confirmMsg().then(({ res }) => {
          this.delGlobal(`${this.apiBaseURL}/delete_chronique/${id}`).then(
            ({ data }) => {
              this.showMsg(data.data);
              this.fetchDataList();
            }
      ...
      fetchDataList() {
        this.fetch_data(`${this.apiBaseURL}/fetch_chronique_malade/${this.refMalade}?page=`);        
      },      
      fetchListSelection() {
        this.editOrFetch(`${this.apiBaseURL}/fetch_list_maladiechronique`).then(
          ({ data }) => {
            var donnees = data.data;
