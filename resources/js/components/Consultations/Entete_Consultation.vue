    ...
    <v-flex md12>
        ...
        <v-card :loading="loading">
          <v-form ref="form" lazy-validation>
            ...
            <v-card-text>
              ...
              <v-textarea readonly type="text" label="Les Maladies Chroniques : " prepend-inner-icon="event" dense
      ...
      <v-dialog v-model="dialog3" max-width="400px" persistent>
        <v-card :loading="loading">
          <v-form ref="form" lazy-validation>
            <v-card-title>
              Cloturer la consultation <v-spacer></v-spacer>
              <v-tooltip bottom color="black">
                <template v-slot:activator="{ on, attrs }">
                  <span v-bind="attrs" v-on="on">
        ...
        <v-flex md12>
          <v-layout>
            ...
            <hr />                    
              ...
              <hr />
              <v-flex md4>
                <v-btn color="#B72C2C" dark :loading="loading" @click="fetchDataListFilter">
                    {{ "Filtrer" }}
                  </v-btn>
              </v-flex> 
                ...
                <template v-slot:default>
                  <thead>
                    <tr>
                      ...
                      <th class="text-left">Parcours</th>
                    ...
                    <tr v-for="item in fetchData" :key="item.id">
                      <td>
                        ...
                        <v-menu bottom rounded offset-y transition="scale-transition">
                          ...
                          <v-list dense width="">
                            ...
                            <v-list-item
                              v-if="(item.cloture == 'NON' && item.duree <= item.nmbreJourConsMvt) || userData.id_role == 2"
                              link @click="showDetailConsultation(item.id, item.noms)">
                              <v-list-item-icon>
                            ...
                            <v-list-item
                              v-if="(item.cloture == 'NON' && item.duree <= item.nmbreJourConsMvt) || userData.id_role == 2"
                              link @click="editDataCloture(item.id)">
                              <v-list-item-icon>
                      ...
                      <td>
                        <v-btn elevation="2" x-small class="white--text"
                          :color="item.parcours == 'Consultation' ? '#00B0F0' : item.parcours == 'Laboratoire' ? '#92D050' : item.parcours == 'Imagerie' ? '#92D050' : item.parcours == 'Kinesitherapie' ? '#70item.nmbreJourConsMvtA0' : item.parcours == 'Dialyse' ? '#ff3333' : item.parcours == 'Pediatrie' ? '#A6A6A6' : item.parcours == 'Hospitalisation' ? '#ff3333' : item.parcours == 'Resultats' ? '#A6A6A6' : 'error'"
                          depressed>
                          {{ item.parcours }}
                        </v-btn>
                      </td>
...
export default {
  ...
  methods: {
    ...
    fetchDataList() {
      ...
      this.fetch_data(`${this.apiBaseURL}/fetch_enteteconsultation?page=`);
    },
    fetchDataListFilter() {
      this.fetch_data(`${this.apiBaseURL}/fetch_enteteconsultation_filter?date1=` + this.svData.date1 + "&date2=" + this.svData.date2 + "&page=");
    },
