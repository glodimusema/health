                                ...
                                <v-menu bottom rounded offset-y transition="scale-transition">
                                  ...
                                  <v-list dense width="">
                                    ...
                                    <v-list-item link @click="printBill(item.id)">
                                        ...
                                        <v-list-item-title style="margin-left: -20px">Voir le Rapport Medical</v-list-item-title>
                                    </v-list-item>
...
export default {
  ...
  created() {
    ...
    this.getCodePatient(this.refDetailCons);
    this.getHopital(this.idHopital); 
  },
  computed: {
  ...
  methods: {
      ...
      validate2() {
        if (this.$refs.form.validate()) {
          ...
          if (this.edit) {
            ...
            this.svData.refPatient = this.refDetailCons;
            this.svData.Hopital = 'DOCS';
            this.svData.author = this.userData.name;
            this.insertOrUpdate(
              `https://health-malamu.net/api/insert_rapportmedical`,
              JSON.stringify(this.svData)
            )
              .then(({ data }) => {
                this.showMsg(data.data);
    ...
    getCodePatient(id) {
        this.editOrFetch(`${this.apiBaseURL}/fetch_single_detailconsultation/${id}`).then(
          ({ data }) => {
            ...
            donnees.map((item) => {
              this.svData.refPatient = item.numeroCarte_malade;
            });
    ...
    fetchDataList() {
      this.fetch_data(`${this.apiBaseURL}/fetch_rapportmedical_cons/${this.refDetailCons}?page=`);
      //
    }
