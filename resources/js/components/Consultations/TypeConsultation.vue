<template>
            ...
            <v-card :loading="loading">
              <v-form ref="form" lazy-validation>
                ...
                <v-card-text>
                  ...
                  <v-text-field type="number" label="Prix Consultation($)" prepend-inner-icon="event" dense
                        :rules="[(v) => !!v || 'Ce champ est requis']" outlined v-model="svData.PrixCons">
                  </v-text-field>
  ...
  export default {
    ...
    methods: {
      ...
      showModal() {
        this.dialog = true;
        this.titleComponent = "Ajout Type Consultation ";
        this.edit = false;
    ...
    //   }, 300),
      onPageChange() {
        this.fetch_data(`${this.apiBaseURL}/fetch_typeconsultation?page=`);
      },
      ...
      validate() {
        if (this.$refs.form.validate()) {
          ...
          this.insertOrUpdate(
            `${this.apiBaseURL}/insert_typeconsultation`,
      ...
      editData(id) {
        this.editOrFetch(`${this.apiBaseURL}/fetch_single_typeconsultation/${id}`).then(
          ({ data }) => {
      ...
      clearP(id) {
        this.confirmMsg().then(({ res }) => {
          this.delGlobal(`${this.apiBaseURL}/delete_typeconsultation/${id}`).then(
            ({ data }) => {
