            ...
            <v-form ref="form" lazy-validation>
              ...
              <v-card-text>
                <v-text-field label="Designation" prepend-inner-icon="extension" dense
                  :rules="[(v) => !!v || 'Ce champ est requis']" outlined v-model="svData.name_periode_cps"></v-text-field>
              </v-card-text>
              <v-card-actions>
                <v-spacer></v-spacer>
...
export default {
  ...
  methods: {
    ...
    testTitle() {
      if (this.edit == true) {
        this.titleComponent = "modification de " + item.name_periode_cps;
      } else {
        this.titleComponent = "Ajout Categorie Vaccin";
      }
    }
    ,
    onPageChange() {
      this.fetch_data(`${this.apiBaseURL}/fetch_periodeCps?page=`);
    },
      fetchAccess() {
    ...
    validate() {
      if (this.$refs.form.validate()) {
        ...
        this.insertOrUpdate(
          `${this.apiBaseURL}/insert_periodeCps`,
    ...
    editData(id) {
      this.editOrFetch(`${this.apiBaseURL}/fetch_single_periodeCps/${id}`).then(
        ({ data }) => {
          var donnees = data.data;
    ...
    clearP(id) {
      this.confirmMsg().then(({ res }) => {
        this.delGlobal(`${this.apiBaseURL}/delete_periodeCps/${id}`).then(
          ({ data }) => {
