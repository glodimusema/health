            ...
            <v-form ref="form" lazy-validation>
              ...
              <v-card-text>
                <v-layout row wrap>
                  ...
                  :rules="[(v) => !!v || 'Ce champ est requis']" outlined v-model="svData.name_periode"></v-text-field>
                    </div>
                  </v-flex>

                  <v-flex xs12 sm12 md12 lg12>
                    <div class="mr-1">
                      <v-text-field type="number" label="Durée(Nbr de Semaines Après la Naissance)" prepend-inner-icon="extension" dense
                  :rules="[(v) => !!v || 'Ce champ est requis']" outlined v-model="svData.duree_periode"></v-text-field>                
                    </div>
                  </v-flex>
...
export default {
  ...
  methods: {
    ...
    showModal() {
      this.dialog = true;
      this.titleComponent = "Ajout Categorie Vaccin";
      this.edit = false;
    ...
    testTitle() {
      if (this.edit == true) {
        this.titleComponent = "modification de " + item.name_periode;
      } else {
        this.titleComponent = "Ajout Categorie Vaccin";
      }
    },
    ...
    ,
    onPageChange() {
      this.fetch_data(`${this.apiBaseURL}/fetch_periodeVac?page=`);
    },
    ...
    validate() {
      if (this.$refs.form.validate()) {
        ...
        this.insertOrUpdate(
          `${this.apiBaseURL}/insert_periodeVac`,
    ...
    editData(id) {
      this.editOrFetch(`${this.apiBaseURL}/fetch_single_periodeVac/${id}`).then(
        ({ data }) => {
          var donnees = data.data;
    ...
    clearP(id) {
      this.confirmMsg().then(({ res }) => {
        this.delGlobal(`${this.apiBaseURL}/delete_periodeVac/${id}`).then(
          ({ data }) => {
