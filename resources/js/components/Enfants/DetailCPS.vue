                    ...
                    <v-card-text>               
                      ...
                      <v-flex xs12 sm12 md12 lg12>
                              <div class="mr-1">
                                  ...
                                  :rules="[(v) => !!v || 'Ce champ est requis']" :items="this.stataData.PeriodeList" item-text="name_periode_cps"
...
<script>
  ...
  export default {
    data() {
      return {
        ...
        refEnteteVac: 0, 
         ...
         // id,refEnteteVac,refPeriode,dateRecu,poids,author
    ...
    methods: {
      ...
      validate() {
        if (this.$refs.form.validate()) {
          ...
          if (this.edit) {
            ...
            this.insertOrUpdate(
              `${this.apiBaseURL}/update_cps/${this.svData.id}`,
      ...
      editData(id) {
        this.editOrFetch(`${this.apiBaseURL}/fetch_single_cps/${id}`).then(
          ({ data }) => {
            var donnees = data.data;
            donnees.map((item) => {  
      ...
      deleteData(id) {
        this.confirmMsg().then(({ res }) => {
          this.delGlobal(`${this.apiBaseURL}/delete_cps/${id}`).then(
            ({ data }) => {
              this.showMsg(data.data);
              this.fetchDataList();
            }
      ...
      fetchDataList() {
        this.fetch_data(`${this.apiBaseURL}/fetch_cps_entetevac/${this.refEnteteVac}?page=`);      
      },      
      fetchListSelection() {
        this.editOrFetch(`${this.apiBaseURL}/fetch_periode_cps2`).then(
          ({ data }) => {
            var donnees = data.data;
