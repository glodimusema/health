...
<!--  -->
                      <v-flex xs12 sm12 md12 lg12>
                              <div class="mr-1">
                                  ...
                                  item-value="id" dense outlined v-model="svData.refPeriode_Peni" chips clearable
                                  >
                                  </v-autocomplete>
                              </div>
                          </v-flex>
                          <v-flex xs12 sm12 md12 lg12>
                              <div class="mr-1">
                                <v-text-field type="date" label="Date Reçu" prepend-inner-icon="extension" :rules="[(v) => !!v || 'Ce champ est requis']" outlined dense
                                  v-model="svData.date_recusPeni"></v-text-field>
                              </div>
                          </v-flex>
                          ...
                          <v-flex xs12 sm12 md12 lg12>
                              <div class="mr-1">
                                <v-text-field type="number" label="Poids" prepend-inner-icon="extension" :rules="[(v) => !!v || 'Ce champ est requis']" outlined dense
                                  v-model="svData.poids_Peni"></v-text-field>
                              </div>
                          </v-flex>
                        ...
                        <template v-slot:default>
                          ...
                          <tbody>
                            <tr v-for="item in fetchData" :key="item.id">
                              ...
                              <td>{{ item.poids_Peni }}</td>
                              <td>{{ item.date_recusPeni }}</td>
                              <td>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      validate() {
        if (this.$refs.form.validate()) {
          ...
          if (this.edit) {
            ...
            this.insertOrUpdate(
              `${this.apiBaseURL}/update_peni_mere/${this.svData.id}`,
            ...
            this.svData.author = this.userData.name;
            this.insertOrUpdate(
              `${this.apiBaseURL}/insert_peni_mere`,
              JSON.stringify(this.svData)
            )
              .then(({ data }) => {
                this.showMsg(data.data);
      ...
      editData(id) {
        this.editOrFetch(`${this.apiBaseURL}/fetch_single_peni_mere/${id}`).then(
          ({ data }) => {
            var donnees = data.data;
            console.log(donnees);
            donnees.map((item) => {  
              this.svData.refCPN = item.refCPN;
              this.svData.refPeriode_Peni = item.refPeriode_Peni;  
              this.svData.date_recusPeni = item.date_recusPeni;
              this.svData.poids_Peni = item.poids_Peni;
      ...
      deleteData(id) {
        this.confirmMsg().then(({ res }) => {
          this.delGlobal(`${this.apiBaseURL}/delete_peni_mere/${id}`).then(
            ({ data }) => {
              this.showMsg(data.data);
              this.fetchDataList();
            }
      ...
      fetchDataList() {
        this.fetch_data(`${this.apiBaseURL}/fetch_peni_mere_entete/${this.refCPN}?page=`);      
      },      
      fetchListSelection() {
        this.editOrFetch(`${this.apiBaseURL}/fetch_periode_peni_mere2`).then(
          ({ data }) => {
            var donnees = data.data;
