            ...
            <v-form ref="form" lazy-validation>
              ...
              <v-card-text>
                <v-layout row wrap>
                  ...
                  :rules="[(v) => !!v || 'Ce champ est requis']" outlined v-model="svData.nom_periode"></v-text-field>
                    </div>
                  </v-flex>

                  <v-flex xs12 sm12 md12 lg12>
                    <div class="mr-1">
                      <v-text-field type="number" label="Durée(Nbr de Semaines Après la DDR)" prepend-inner-icon="extension" dense
                  :rules="[(v) => !!v || 'Ce champ est requis']" outlined v-model="svData.dure_semsuie"></v-text-field>                
                    </div>
                  </v-flex>
...
export default {
  ...
  methods: {
    ...
    showModal() {
      this.dialog = true;
      this.titleComponent = "Ajout Période Vaccin Mère";
      this.edit = false;
    ...
    testTitle() {
      if (this.edit == true) {
        this.titleComponent = "modification de " + item.nom_periode;
      } else {
        this.titleComponent = "Ajout Période Vaccin Mère";
      }
    },
    ...
    ,
    onPageChange() {
      this.fetch_data(`${this.apiBaseURL}/fetch_periode_vacciniere?page=`);
    },
    ...
    validate() {
      if (this.$refs.form.validate()) {
        ...
        this.insertOrUpdate(
          `${this.apiBaseURL}/insert_periode_vacciniere`,
    ...
    editData(id) {
      this.editOrFetch(`${this.apiBaseURL}/fetch_single_periode_vacciniere/${id}`).then(
        ({ data }) => {
          var donnees = data.data;
    ...
    clearP(id) {
      this.confirmMsg().then(({ res }) => {
        this.delGlobal(`${this.apiBaseURL}/delete_periode_vacciniere/${id}`).then(
          ({ data }) => {
