            ...
            <v-form ref="form" lazy-validation>
              ...
              <v-card-text>
                <v-layout row wrap>
                  ...
                  :rules="[(v) => !!v || 'Ce champ est requis']" outlined v-model="svData.name_periode_Sp"></v-text-field>
                    </div>
                  </v-flex>
...
export default {
  ...
  methods: {
    ...
    showModal() {
      this.dialog = true;
      this.titleComponent = "Ajout Periode SP";
      this.edit = false;
    ...
    testTitle() {
      if (this.edit == true) {
        this.titleComponent = "modification de " + item.name_periode_Sp;
      } else {
        this.titleComponent = "Ajout Periode SP";
      }
    }
    ,
    onPageChange() {
      this.fetch_data(`${this.apiBaseURL}/fetch_periode_sp?page=`);
    },
    ...
    validate() {
      if (this.$refs.form.validate()) {
        ...
        this.insertOrUpdate(
          `${this.apiBaseURL}/insert_periode_sp`,
    ...
    editData(id) {
      this.editOrFetch(`${this.apiBaseURL}/fetch_single_periode_sp/${id}`).then(
        ({ data }) => {
          var donnees = data.data;
    ...
    clearP(id) {
      this.confirmMsg().then(({ res }) => {
        this.delGlobal(`${this.apiBaseURL}/delete_periode_sp/${id}`).then(
          ({ data }) => {
