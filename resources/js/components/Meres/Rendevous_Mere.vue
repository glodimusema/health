                      ...
                      <v-flex xs12 sm12 md12 lg12>
                          ...
                          <v-flex xs12 sm12 md12 lg12>
                              <div class="mr-1">
                                <v-text-field type="date" label="Date Rendez-vous" prepend-inner-icon="extension" :rules="[(v) => !!v || 'Ce champ est requis']" outlined dense
                                  v-model="svData.date_RDV"></v-text-field>
                              </div>
                          </v-flex>                          
                        ...
                        <div class="mr-1">
                            <v-select label="Compteur" :items="[
                              ...
                              :rules="[(v) => !!v || 'Ce champ est requis']" outlined dense item-text="designation"
                              item-value="designation" v-model="svData.compteurRDV"></v-select>
                          </div>
                        </v-flex>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      validate() {
        if (this.$refs.form.validate()) {
          ...
          if (this.edit) {
            ...
            this.insertOrUpdate(
              `${this.apiBaseURL}/update_rendezvous_vac_mere/${this.svData.id}`,
            ...
            this.svData.author = this.userData.name;
            this.insertOrUpdate(
              `${this.apiBaseURL}/insert_rendezvous_vac_mere`,
              JSON.stringify(this.svData)
            )
              .then(({ data }) => {
                this.showMsg(data.data);
      ...
      editData(id) {
        this.editOrFetch(`${this.apiBaseURL}/fetch_single_rendezvous_vac_mere/${id}`).then(
          ({ data }) => {
            var donnees = data.data;
            donnees.map((item) => {  
              ...
              this.svData.compteurRDV = item.compteurRDV;
      ...
      deleteData(id) {
        this.confirmMsg().then(({ res }) => {
          this.delGlobal(`${this.apiBaseURL}/delete_rendezvous_vac_mere/${id}`).then(
            ({ data }) => {
              this.showMsg(data.data);
              this.fetchDataList();
            }
      ...
      fetchDataList() {
        this.fetch_data(`${this.apiBaseURL}/fetch_rdv_vaccin_mere_entete/${this.refCPN}?page=`);
