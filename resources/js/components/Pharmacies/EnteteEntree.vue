    ...
    <v-flex md12>
      ...
      <v-layout row wrap>
        <v-flex xs12 sm12 md6 lg6>
          <div class="mr-1">
            <router-link :to="'#'">Approvisionnements Medicaments</router-link>
          </div>
        </v-flex>
      </v-layout>
                ...
                <template v-slot:default>
                  <thead>
                    <tr>
                      ...
                      <th class="text-left">Date Entrée</th>
...
import DetailEntree from './DetailEntree.vue';
...
export default {
  ...
  methods: {
    ...
    validate() {
      if (this.$refs.form.validate()) {
        ...
        if (this.edit) {
          ...
          this.insertOrUpdate(
            `${this.apiBaseURL}/update_enteteentree/${this.svData.id}`,
          ...
          this.svData.author = this.userData.name;
          this.insertOrUpdate(
            `${this.apiBaseURL}/insert_enteteentree`,
            JSON.stringify(this.svData)
          )
            .then(({ data }) => {
              this.showMsg(data.data);
    ...
    editData(id) {
      this.editOrFetch(`${this.apiBaseURL}/fetch_single_enteteentree/${id}`).then(
        ({ data }) => {
          var donnees = data.data;
          donnees.map((item) => {
    ...
    deleteData(id) {
      this.confirmMsg().then(({ res }) => {
        this.delGlobal(`${this.apiBaseURL}/delete_enteteentree/${id}`).then(
          ({ data }) => {
            this.showMsg(data.data);
            this.fetchDataList();
          }
    ...
    fetchDataList() {
      this.fetch_data(`${this.apiBaseURL}/fetch_enteteentree?page=`);
    },
    ...
    showDetailEntree(refEnteteEntree, name) {
      ...
      if (refEnteteEntree != '') {
        ...
        this.$refs.DetailEntree.$data.etatModal = true;
        this.$refs.DetailEntree.$data.refEnteteEntree = refEnteteEntree;
        this.$refs.DetailEntree.$data.svData.refEnteteEntree = refEnteteEntree;
        this.$refs.DetailEntree.fetchDataList();
        this.$refs.DetailEntree.fetchListSelection();
        this.fetchDataList();

        this.$refs.DetailEntree.$data.titleComponent =
