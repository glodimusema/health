<template>
                      ...
                      <v-form ref="form" lazy-validation>
                        ...
                        <v-card-text>
                          ...
                          <v-text-field type="number" label="Quantité " prepend-inner-icon="event" dense
                            :rules="[(v) => !!v || 'Ce champ est requis']" outlined v-model="svData.qte_besoin">
                          </v-text-field>
    
                          <!-- <v-text-field type="number" label="Prix Unitaire ($)" prepend-inner-icon="event" dense
                            :rules="[(v) => !!v || 'Ce champ est requis']" outlined v-model="svData.pu_besoin">
                          </v-text-field> -->


                          <v-text-field label="Observation" prepend-inner-icon="event" dense
                            :rules="[(v) => !!v || 'Ce champ est requis']" outlined v-model="svData.observation_besoin">
                          </v-text-field>
                            ...
                            <template v-slot:default>
                              <thead>
                                <tr>
                                  ...
                                  <th class="text-left">DateBesoin</th>
                              ...
                              <tbody>
                                <tr v-for="item in fetchData" :key="item.id">
                                  ...
                                  <td>{{ item.qte_besoin }}</td>
                                  <td>{{ item.observation_besoin }}</td>
    ...
    // 
      ...
      methods: {
        ...
        validate() {
          if (this.$refs.form.validate()) {
            ...
            if (this.edit) {
              ...
              this.svData.pu_besoin=0;
              this.insertOrUpdate(
                `${this.apiBaseURL}/update_detail_besoin/${this.svData.id}`,
              ...
              this.svData.pu_besoin=0;
              this.insertOrUpdate(
                `${this.apiBaseURL}/insert_detail_besoin`,
                JSON.stringify(this.svData)
              )
                .then(({ data }) => {
                  this.showMsg(data.data);
        ...
        editData(id) {
          this.editOrFetch(`${this.apiBaseURL}/fetch_single_detail_besoin/${id}`).then(
            ({ data }) => {
              ...
              donnees.map((item) => {
                ...
                this.svData.pu_besoin = item.pu_besoin;
                this.svData.qte_besoin = item.qte_besoin;
                this.svData.observation_besoin = item.observation_besoin;
              });
        ...
        deleteData(id) {
          this.confirmMsg().then(({ res }) => {
            this.delGlobal(`${this.apiBaseURL}/delete_detail_besoin/${id}`).then(
              ({ data }) => {
                this.showMsg(data.data);
                this.fetchDataList();
              }
        ...
        fetchDataList() {
          this.fetch_data(`${this.apiBaseURL}/fetch_detail_entete_besoin/${this.refEnteteVente}?page=`);
