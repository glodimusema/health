                ...
                <v-card :loading="loading">
                  <v-form ref="form" lazy-validation>
                    ...
                    <v-card-text>
                      ...
                      <v-text-field label="Numéro Lot " prepend-inner-icon="event" dense
                        :rules="[(v) => !!v || 'Ce champ est requis']" outlined v-model="svData.numeroLot">
                      </v-text-field>
...
export default {
  ...
  methods: {
    ...
    validate() {
      if (this.$refs.form.validate()) {
        ...
        if (this.edit) {
          ...
          this.insertOrUpdate(
            `${this.apiBaseURL}/update_detailentree/${this.svData.id}`,
          ...
          this.svData.author = this.userData.name;
          this.insertOrUpdate(
            `${this.apiBaseURL}/insert_detailentree`,
            JSON.stringify(this.svData)
          )
            .then(({ data }) => {
              this.showMsg(data.data);
    ...
    // s'id','refEnteteEntree','refmedicament','puEntree','qteEntree','author'
    //   this.fetchDataList();
    // }, 300),

    editData(id) {
      this.editOrFetch(`${this.apiBaseURL}/fetch_single_detailentree/${id}`).then(
        ({ data }) => {
          var donnees = data.data;
          donnees.map((item) => {
            ...
            this.svData.numeroLot = item.numeroLot;
    ...
    deleteData(id) {
      this.confirmMsg().then(({ res }) => {
        this.delGlobal(`${this.apiBaseURL}/delete_detailentree/${id}`).then(
          ({ data }) => {
            this.showMsg(data.data);
            this.fetchDataList();
          }
    ...
    fetchDataList() {
      this.fetch_data(`${this.apiBaseURL}/fetch_detail_enteteentree/${this.refEnteteEntree}?page=`);
    },
    ...
    getPrice(id) {
      this.editOrFetch(`${this.apiBaseURL}/fetch_single_medicament2/${id}`).then(
        ({ data }) => {
          var donnees = data.data;
          donnees.map((item) => {
            this.svData.puEntree = item.pu_medicament;
