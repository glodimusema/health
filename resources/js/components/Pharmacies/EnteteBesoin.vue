<template>    
            ...
            <v-card-text>
              ...
              <div>
                ...
                <v-flex md12>
                    ...
                    <!-- FeuilleFacturation -->
                    <DetailBesoin ref="DetailBesoin" />

                    <v-dialog v-model="dialog" max-width="600px" persistent>
                    <v-card :loading="loading">
                        <v-form ref="form" lazy-validation>
                        <v-card-title>
                            Création EB<v-spacer></v-spacer>
                        ...
                        <v-text-field type="date" label="Date Besoin" prepend-inner-icon="event" dense
                            :rules="[(v) => !!v || 'Ce champ est requis']" outlined v-model="svData.date_besoin">
                        </v-text-field>
                                ...
                                <tr v-for="item in fetchData" :key="item.id">
                                  <td>
                                    <v-menu bottom rounded offset-y transition="scale-transition">
                                        ...
                                        <v-list-item link @click="showDetailBesoin(item.id, item.noms)">
                                            <v-list-item-icon>
                                        ...
                                        <v-list-item link @click="printBill(item.id)">
                                            ...
                                            <v-list-item-title style="margin-left: -20px">Fiche de demande
                                            </v-list-item-title>
                                        </v-list-item>
                                    ...
                                    <td>{{ item.contact }}</td>
                                    <td>{{ item.date_besoin | formatDate }}</td>
  ...
  import DetailBesoin from './DetailBesoin.vue'; 
  ...
  export default {
    ...
    methods: {
      ...
      validate() {
        if (this.$refs.form.validate()) {
          ...
          if (this.edit) {
            ...
            this.insertOrUpdate(
              `${this.apiBaseURL}/update_entetebesoin/${this.svData.id}`,
            ...
            this.svData.refMouvement=this.refMouvement;
            this.insertOrUpdate(
              `${this.apiBaseURL}/insert_entetebesoin`,
              JSON.stringify(this.svData)
            )
              .then(({ data }) => {
                this.showMsg(data.data);
      ...
      printBill(id) {
        window.open(`${this.apiBaseURL}/pdf_besoinservices_data?id=` + id);
      },
  
      editData(id) {
        this.editOrFetch(`${this.apiBaseURL}/fetch_single_entetebesoin/${id}`).then(
          ({ data }) => {
            ...
            donnees.map((item) => {
              ...
              this.svData.date_besoin = item.date_besoin;
      ...
      deleteData(id) {
        this.confirmMsg().then(({ res }) => {
          this.delGlobal(`${this.apiBaseURL}/delete_entetebesoin/${id}`).then(
            ({ data }) => {
              this.showMsg(data.data);
              this.fetchDataList();
            }
      ...
      fetchDataList() {
        this.fetch_data(`${this.apiBaseURL}/fetch_besoin_mouvement/${this.refMouvement}?page=`);
      },
    fetchListService() {
    ...
    showDetailBesoin(refEnteteVente, name) {

      if (refEnteteVente != '') {

        this.$refs.DetailBesoin.$data.etatModal = true;
        this.$refs.DetailBesoin.$data.refEnteteVente = refEnteteVente;
        this.$refs.DetailBesoin.$data.svData.refEnteteVente = refEnteteVente;
        this.$refs.DetailBesoin.fetchDataList();
        this.$refs.DetailBesoin.fetchListSelection();
        this.fetchDataList();

        this.$refs.DetailBesoin.$data.titleComponent =
          "Detail Besoin pour " + name;
