                ...
                <v-dialog v-model="dialog" max-width="1200px" persistent>
                  <v-card :loading="loading">
                    <v-form ref="form" lazy-validation>
                      <v-card-title>
                        Certificat d'Aptidude Physique <v-spacer></v-spacer>
                          ...
                          <v-flex xs12 sm12 md6 lg6>
                                    <div class="mr-1">
                                        <v-select label="Etat de Santé *" :items="[
                                            ...
                                            v-model="svData.etatDeSante"></v-select>
                                    </div>
                                </v-flex>
...
export default {
  ...
  methods: {
    ...
    validate() {
      if (this.$refs.form.validate()) {
        ...
        if (this.edit) {
          ...
          this.insertOrUpdate(
            `${this.apiBaseURL}/update_AptPhysique/${this.svData.id}`,
          ...
          this.svData.author = this.userData.name;
          this.insertOrUpdate(
            `${this.apiBaseURL}/insert_AptPhysique`,
            JSON.stringify(this.svData)
          )
            .then(({ data }) => {
              this.showMsg(data.data);
    ...
    printBill(id) {
      window.open(`${this.apiBaseURL}/pdf_aptitudephysique_data?id=` + id);
    },
    ...
    editData(id) {
      this.editOrFetch(`${this.apiBaseURL}/fetch_single_AptPhysique/${id}`).then(
        ({ data }) => {
          var donnees = data.data;
          donnees.map((item) => {
            ...
            this.svData.etatDeSante = item.etatDeSante;
    ...
    deleteData(id) {
      this.confirmMsg().then(({ res }) => {
        this.delGlobal(`${this.apiBaseURL}/delete_AptPhysique/${id}`).then(
          ({ data }) => {
            this.showMsg(data.data);
            this.fetchDataList();
          }
    ...
    fetchDataList() {
       this.fetch_data(`${this.apiBaseURL}/fetch_aptitudephysique_entete/${this.refAttestation}?page=`);
