            ...
            <v-form ref="form" lazy-validation>
              ...
              <v-card-text>
                <v-text-field label="Designation" prepend-inner-icon="extension" dense
                  :rules="[(v) => !!v || 'Ce champ est requis']" outlined v-model="svData.nomTypeMachine"></v-text-field>

                <v-textarea label="Description" prepend-inner-icon="event" dense
...
export default {
  ...
  methods: {
    ...
    showModal() {
      this.dialog = true;
      this.titleComponent = "Ajout Type Machine ";
      this.edit = false;
    ...
    testTitle() {
      if (this.edit == true) {
        this.titleComponent = "modification de " + item.nomTypeMachine;
      } else {
        this.titleComponent = "Ajout Type Machine ";
      }
    }
    ...
    //   }, 300),
    onPageChange() {
      this.fetch_data(`${this.apiBaseURL}/fetch_all_typeMarchine?page=`);
    },
    ...
    validate() {
      if (this.$refs.form.validate()) {
        ...
        this.insertOrUpdate(
          `${this.apiBaseURL}/inserttypeMarchine`,
    ...
    editData(id) {
      this.editOrFetch(`${this.apiBaseURL}/fetch_sigle_typeMarchine/${id}`).then(
        ({ data }) => {
          var donnees = data.data;
    ...
    clearP(id) {
      this.confirmMsg().then(({ res }) => {
        this.delGlobal(`${this.apiBaseURL}/delete_typeMarchine/${id}`).then(
          ({ data }) => {
