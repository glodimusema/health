          ...
          <v-form ref="form" lazy-validation>
            ...
            <v-card-text>
              <v-layout row wrap>
                ...
                <div class="mr-1">
                  <v-autocomplete label="Selectionnez le Nom" prepend-inner-icon="person"
                    :rules="[(v) => !!v || 'Ce champ est requis']" :items="listeMedecin" item-text="noms_medecin" item-value="id" dense
          ...
          <v-layout>
            ...
            <v-flex md2>
              ...
              <v-tooltip bottom color="black">
                <template v-slot:activator="{ on, attrs }">
                  <span v-bind="attrs" v-on="on">
                    <v-btn @click="exportation" fab color="#B72C2C" dark>
...
export default {
  ...
  methods: {
    ...
    exportation() {
      window.open(`${this.baseURL}/exportation`);
      // this.editOrFetch(`${this.baseURL}/exportation`).then(
      //   ({ data }) => {
      //     // var donnees = data.data;
      //     // donnees.map((item) => {
      //     //   this.titleComponent = "modification de " + item.name;
      ...
      getDataAgent(id) {
        this.editOrFetch(`${this.apiBaseURL}/fetch_single_medecin/${id}`).then(
          ({ data }) => {
            var donnees = data.data;
            donnees.map((item) => {
              this.svData.name = item.noms_medecin; 
              this.svData.adresse = item.nomQuartier+';'+item.nomAvenue; 
              this.svData.email = item.mail_medecin;
              this.svData.telephone = item.contact_medecin; 
