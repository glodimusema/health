...
export default {
  ...
  methods: {
    ...
    editData(id) {
      this.editOrFetch(`${this.apiBaseURL}/fetch_single_video/${id}`).then(
        ({ data }) => {
    ...
    clearP(id) {
      this.confirmMsg().then(({ res }) => {
        this.delGlobal(`${this.apiBaseURL}/delete_video/${id}`).then(
          ({ data }) => {
    ...
    editTitleModal(id) {
      this.editOrFetch(`${this.apiBaseURL}/fetch_single_video/${id}`).then(
        ({ data }) => {
          var donnees = data.data;
          donnees.map((item) => {
            this.titleComponent = "modification de vidéo " + item.titre;
          });
        }
      );
    },
