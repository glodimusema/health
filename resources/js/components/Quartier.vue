<template>
  ...
  export default {
    ...
    methods: {
      ...
      // ,
      onPageChange() {
        this.fetch_data(`${this.apiBaseURL}/ad1/fetch_quartier?page=`);
      },
      ...
      validate() {
        if (this.$refs.form.validate()) {
          ...
          this.insertOrUpdate(
            `${this.apiBaseURL}/ad1/insert_quartier`,
      ...
      editData(id) {
        this.editOrFetch(`${this.apiBaseURL}/ad1/fetch_single_quartier/${id}`).then(
          ({ data }) => {
            var donnees = data.data;
  
            donnees.map((item) => {
              this.titleComponent = "modification de " + item.nomQuartier;
      ...
      clearP(id) {
        this.confirmMsg().then(({ res }) => {
          this.delGlobal(`${this.apiBaseURL}/ad1/delete_quartier/${id}`).then(
            ({ data }) => {
