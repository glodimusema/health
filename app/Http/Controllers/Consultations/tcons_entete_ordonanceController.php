...
use App\Models\Consultations\tcons_entete_ordonance;
use DB;

class tcons_entete_ordonanceController extends Controller
{
    public function index()
    {
        return 'hello';
    }
//tcons_entete_ordonance
    ...
    //
        ...
        if (!is_null($request->get('query'))) {
            ...
            ->where([
                ['noms', 'like', '%'.$query.'%'],
                ['tcons_entete_ordonance.deleted','NON']
            ])            
            ->orderBy("tcons_entete_ordonance.id", "desc")          
            ->paginate(10);
            ...
            "contactPersRef_malade","organisation_malade","numeroCarte_malade",
            "dateExpiration_malade","PrixCons")
            ->where([['tcons_entete_ordonance.deleted','NON']])
            ->selectRaw('TIMESTAMPDIFF(YEAR, dateNaissance_malade, CURDATE()) as age_malade')
            ->orderBy("tcons_entete_ordonance.id", "desc")
            ->paginate(10);
                return response()->json([
    ...
    function fetch_max_enteteOrdonance_Cons(Request $request)
    {
        $refDetailCons = $request->id;

        $data1 = tcons_entete_ordonance::create([
            'refDetailCons'       =>  $refDetailCons,
            'date_ordonance'       =>  date('Y-m-d'),                                        
            'author'       =>  $request->author
        ]);


        $idmax=0;
        $maxid = DB::table('tcons_entete_ordonance')
        ->join('tdetailconsultation','tdetailconsultation.id','=','tcons_entete_ordonance.refDetailCons')
        ->join('tenteteconsulter','tenteteconsulter.id','=','tdetailconsultation.refEnteteCons')
        ->join('ttypeconsultation','ttypeconsultation.id','=','tdetailconsultation.refTypeCons')
        ->join('tmedecin','tmedecin.id','=','tenteteconsulter.refMedecin')
        ...
        ->join('provinces' , 'provinces.id','=','villes.idProvince')
        ->join('pays' , 'pays.id','=','provinces.idPays')        
        ->selectRaw('MAX(tcons_entete_ordonance.id) as code_enteteordo')
        ->where('tcons_entete_ordonance.refDetailCons', $refDetailCons)
        ->get();
        foreach ($maxid as $list) {
            $idmax= $list->code_enteteordo;
        }
        ...
        "dateExpiration_malade","PrixCons")
        ->selectRaw('TIMESTAMPDIFF(YEAR, dateNaissance_malade, CURDATE()) as age_malade')
        ->where('tcons_entete_ordonance.id', $idmax)
        ->get();
        //
        return response()->json([
        'data' => $data,
   ...
   //'id','refDetailCons','date_ordonance','author'
    function insert_orientations(Request $request)
    {
        ...
        $data = tcons_entete_ordonance::create([
            'refDetailCons'       =>  $request->refDetailCons,
            'date_ordonance'    =>  $request->date_ordonance,                            
            'author'       =>  $request->author
        ]);
        return response()->json([
    ...
    {
        $data = tcons_entete_ordonance::where('id', $id)->update([
            'refDetailCons'       =>  $request->refDetailCons,
            'date_ordonance'    =>  $request->date_ordonance,                            
            'author'       =>  $request->author
        ]);
    ...
    {
        $data = tcons_entete_ordonance::where('id',$id)->delete();
        return response()->json([
