...
use App\Models\Laboratoire\tlabo_detail_germe;
use DB;

class tlabo_detail_germeController extends Controller
{
    ...
    { 
        ...
        if (!is_null($request->get('query'))) {
            ...
            'dateResultat','aspectmacro','refResultatBacterie','refGerme','nom_germe',
            ...
            ->where([
                ['noms', 'like', '%'.$query.'%'],          
                ['tlabo_detail_germe.deleted','NON']
            ])             
            ->orderBy("tlabo_detail_germe.id", "desc")          
            ->paginate(10);
    ...
    function fetch_single_data($id)
    {
        ...
        "dateExpiration_malade","PrixCons")
        ->selectRaw('TIMESTAMPDIFF(YEAR, dateNaissance_malade, CURDATE()) as age_malade')
        ->where('tlabo_detail_germe.id', $id)
            ->get();
   ...
   //'id','refResultatBacterie','refGerme','author'
    ...
    function insert_data(Request $request)
    {
        ...
        $data = tlabo_detail_germe::create([
            'refResultatBacterie'       =>  $request->refResultatBacterie,
            'refGerme'    =>  $request->refGerme,                  
    ...
    {
        $data = tlabo_detail_germe::where('id', $id)->update([
            'refResultatBacterie'       =>  $request->refResultatBacterie,
            'refGerme'    =>  $request->refGerme,                  
            'author'       =>  $request->author
        ]);
    ...
    {
        $data = tlabo_detail_germe::where('id',$id)->delete();
        return response()->json([
