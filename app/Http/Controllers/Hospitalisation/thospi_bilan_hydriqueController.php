...
use App\Models\Hospitalisation\thospi_bilan_hydrique;
use DB;

class thospi_bilan_hydriqueController extends Controller
{
    ...
    { 
        ...
        if (!is_null($request->get('query'))) {
            ...
            $data = DB::table('thospi_bilan_hydrique')
            ...
            //
            ->select("thospi_bilan_hydrique.id","dateBilan","totalEntree","totalSortie",
            "hydrique","thospi_bilan_hydrique.poids as poidsHydrique",
            "thospi_bilan_hydrique.author",'dateEntree','diagnosticEntree','thospitalisation.observations',
            'dateHospi','refDetailCons',"refLit",'nom_lit','refSalle',"nom_salle","PrixSalle",
            "refEnteteCons","refTypeCons","plainte",
            ...
            ->selectRaw('ROUND((((4*thospi_bilan_hydrique.poids)+7)/(90+thospi_bilan_hydrique.poids)),2)    as sc')
            ->selectRaw('ROUND(((((4*thospi_bilan_hydrique.poids)+7)/(90+thospi_bilan_hydrique.poids))*500),2) as PertesInsensible')            
            ->where([
                ['noms', 'like', '%'.$query.'%'],
                ['thospi_bilan_hydrique.deleted','NON']
            ])            
            ->orderBy("thospi_bilan_hydrique.id", "desc")          
            ->paginate(10);
            ...
            ->selectRaw('ROUND((((4*thospi_bilan_hydrique.poids)+7)/(90+thospi_bilan_hydrique.poids)),2)    as sc')
            ->selectRaw('ROUND(((((4*thospi_bilan_hydrique.poids)+7)/(90+thospi_bilan_hydrique.poids))*500),2) as PertesInsensible')  
            ->where([['thospi_bilan_hydrique.deleted','NON']])          
            ->orderBy("thospi_bilan_hydrique.id", "desc")
            ->paginate(10);
                return response()->json([
                    'data'  => $data,
    ...
    public function fetch_bilan_hydrique(Request $request,$refHospi)
    { 
    ...
    function fetch_single_bilan_hydrique($id)
    {
        ...
        ->selectRaw('ROUND((((4*thospi_bilan_hydrique.poids)+7)/(90+thospi_bilan_hydrique.poids)),2)    as sc')
        ->selectRaw('ROUND(((((4*thospi_bilan_hydrique.poids)+7)/(90+thospi_bilan_hydrique.poids))*500),2) as PertesInsensible')            
        ->where('thospi_bilan_hydrique.id', $id)
        ->get();
    ...
    function insert_bilan_hydrique(Request $request)
    {
        ...
        $data = thospi_bilan_hydrique::create([
            'refHospi'       =>  $request->refHospi,
            'dateBilan'    =>  $request->dateBilan,           
            'totalEntree'       =>  $request->totalEntree,
            'totalSortie'    =>  $request->totalSortie,
            'hydrique'    =>  $request->hydrique,
            'poids'    =>  $request->poids,
    ...
    {
        $data = thospi_bilan_hydrique::where('id', $id)->update([
            'refHospi'       =>  $request->refHospi,
            'dateBilan'    =>  $request->dateBilan,           
            'totalEntree'       =>  $request->totalEntree,
            'totalSortie'    =>  $request->totalSortie,
    ...
    {
        $data = thospi_bilan_hydrique::where('id',$id)->delete();
        return response()->json([
