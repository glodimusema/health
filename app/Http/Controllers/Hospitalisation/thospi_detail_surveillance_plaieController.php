...
use App\Models\Hospitalisation\thospi_detail_surveillance_plaie;
use DB;

class thospi_detail_surveillance_plaieController extends Controller
{
    ...
    { 
        ...
        if (!is_null($request->get('query'))) {
            ...
            $data = DB::table('thospi_detail_surveillance_plaie')
            ->join('thospi_surveillance_plaie','thospi_surveillance_plaie.id','=','thospi_detail_surveillance_plaie.refSurvPlaie')
            ...
            //
            ->select("thospi_detail_surveillance_plaie.id","dateSurv","surfaceCm","profondeurMin",
            "Pics","BVA","protocole","localisation","pourcentageNoire","pourcentageMarron",  
            "pourcentageNoire","pourcentageMarron","pourcentageJaune","pourcentageRouge",
            "pourcentageRose","thospi_detail_surveillance_plaie.author","refHospi","refTypePlaie","refSurvPlaie",
            ...
            ->where([
                ['noms', 'like', '%'.$query.'%'],
                ['thospi_detail_surveillance_plaie.deleted','NON']
            ])            
            ->orderBy("thospi_detail_surveillance_plaie.id", "desc")          
            ->paginate(10);
            ...
            "dateExpiration_malade","PrixCons")
            ->selectRaw('TIMESTAMPDIFF(YEAR, dateNaissance_malade, CURDATE()) as age_malade')   
            ->where([['thospi_detail_surveillance_plaie.deleted','NON']])          
            ->orderBy("thospi_detail_surveillance_plaie.id", "desc")
            ->paginate(10);
                return response()->json([
                    'data'  => $data,
    ...
    public function fetch_surveillance_plaie(Request $request,$refSurvPlaie)
    { 
    ...
    function fetch_single_surveillance_plaie($id)
    {
        ...
        "dateExpiration_malade","PrixCons")
        ->selectRaw('TIMESTAMPDIFF(YEAR, dateNaissance_malade, CURDATE()) as age_malade')               
        ->where('thospi_detail_surveillance_plaie.id', $id)
                ->get();
    ...
    {
        $data = thospi_detail_surveillance_plaie::create([
            'refSurvPlaie'       =>  $request->refSurvPlaie,
            'dateSurv'    =>  $request->dateSurv,
            'surfaceCm'    =>  $request->surfaceCm,
            'profondeurMin'       =>  $request->profondeurMin,
            'Pics'    =>  $request->Pics,
            'BVA'    =>  $request->BVA,
    ...
    {
        $data = thospi_detail_surveillance_plaie::where('id', $id)->update([
            'refSurvPlaie'       =>  $request->refSurvPlaie,
            'dateSurv'    =>  $request->dateSurv,
            'surfaceCm'    =>  $request->surfaceCm,
            'profondeurMin'       =>  $request->profondeurMin,
    ...
    {
        $data = thospi_detail_surveillance_plaie::where('id',$id)->delete();
        return response()->json([
