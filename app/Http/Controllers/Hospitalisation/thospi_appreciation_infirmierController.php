...
use App\Models\Hospitalisation\thospi_appreciation_infirmier;
use DB;

class thospi_appreciation_infirmierController extends Controller
{
    ...
    { 
        ...
        if (!is_null($request->get('query'))) {
            ...
            ->where([
                ['noms', 'like', '%'.$query.'%'],
                ['thospi_appreciation_infirmier.deleted','NON']
            ])            
            ->orderBy("thospi_appreciation_infirmier.id", "desc")          
            ->paginate(10);
            ...
            "dateExpiration_malade","PrixCons")
            ->selectRaw('TIMESTAMPDIFF(YEAR, dateNaissance_malade, CURDATE()) as age_malade')   
            ->where([['thospi_appreciation_infirmier.deleted','NON']])         
            ->orderBy("thospi_appreciation_infirmier.id", "desc")
            ->paginate(10);
                return response()->json([
                    'data'  => $data,
    ...
    public function fetch_appreciation_infirmier(Request $request,$refHospi)
    { 
    ...
    function fetch_single_appreciation_infirmier($id)
    {
        ...
        $data = DB::table('thospi_appreciation_infirmier')
            ...
            "dateExpiration_malade","PrixCons")
            ->selectRaw('TIMESTAMPDIFF(YEAR, dateNaissance_malade, CURDATE()) as age_malade')        
            ->where('thospi_appreciation_infirmier.id', $id)
            ->get();
    ...
    function insert_appreciation_infirmier(Request $request)
    {
       
        $data = thospi_appreciation_infirmier::create([
            'refHospi'       =>  $request->refHospi,
            'degreSatisfaction'    =>  $request->degreSatisfaction,
            'remarques'       =>  $request->remarques,
            'sugestion'    =>  $request->sugestion,
    ...
    function update_appreciation_infirmier(Request $request, $id)
    {
        $data = thospi_appreciation_infirmier::where('id', $id)->update([
            'refHospi'       =>  $request->refHospi,
            'degreSatisfaction'    =>  $request->degreSatisfaction,
            'remarques'       =>  $request->remarques,
            'sugestion'    =>  $request->sugestion,
    ...
    function delete_appreciation_infirmier($id)
    {
        $data = thospi_appreciation_infirmier::where('id',$id)->delete();
        return response()->json([
