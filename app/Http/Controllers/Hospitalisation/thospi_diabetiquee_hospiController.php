...
use App\Models\Hospitalisation\thospi_diabetiquee_hospi;
use DB;

class thospi_diabetiquee_hospiController extends Controller
{
    ...
    { 
        ...
        if (!is_null($request->get('query'))) {
            ...
            $data = DB::table('thospi_diabetiquee_hospi')
            ->join('thospi_surveillance_hospie','thospi_surveillance_hospie.id','=','thospi_diabetiquee_hospi.refSurvHospi')
            ...
            //
            ->select("thospi_diabetiquee_hospi.id","dateDiab","glycemieMatin","doseMatin",
            "siteMatin","ObservationMatin","glycemieSoir","doseSoir",'siteSoir',"observationSoir",
            "medecinAssistant","thospi_diabetiquee_hospi.author","refHospi","refSurvHospi",
            ...
            ->where([
                ['noms', 'like', '%'.$query.'%'],
                ['thospi_diabetiquee_hospi.deleted','NON']
            ])            
            ->orderBy("thospi_diabetiquee_hospi.id", "desc")          
            ->paginate(10);
            ...
            "dateExpiration_malade","PrixCons")
            ->selectRaw('TIMESTAMPDIFF(YEAR, dateNaissance_malade, CURDATE()) as age_malade')   
            ->where([['thospi_diabetiquee_hospi.deleted','NON']])          
            ->orderBy("thospi_diabetiquee_hospi.id", "desc")
            ->paginate(10);
                return response()->json([
                    'data'  => $data,
    ...
    public function fetch_diabetique_hospie(Request $request,$refSurvHospi)
    { 
    ...
    function fetch_single_diabetique($id)
    {
        ...
        "dateExpiration_malade","PrixCons")
        ->selectRaw('TIMESTAMPDIFF(YEAR, dateNaissance_malade, CURDATE()) as age_malade')            
        ->where('thospi_diabetiquee_hospi.id', $id)
                ->get();
    ...
    function insert_diabetique_hospie(Request $request)
    {

        $data = thospi_diabetiquee_hospi::create([
            'refSurvHospi'       =>  $request->refSurvHospi,
            'dateDiab'    =>  $request->dateDiab,
            'glycemieMatin'    =>  $request->glycemieMatin,
            'doseMatin'       =>  $request->doseMatin,
            'siteMatin'    =>  $request->siteMatin,
            'ObservationMatin'    =>  $request->ObservationMatin,
            'glycemieSoir'    =>  $request->glycemieSoir,
            'doseSoir'    =>  $request->doseSoir,
            'doseMatin'       =>  $request->doseMatin,
            'siteSoir'    =>  $request->siteSoir,
            'observationSoir'    =>  $request->observationSoir,
    ...
    function update_diabetique_hospie(Request $request, $id)
    {
        $data = thospi_diabetiquee_hospi::where('id', $id)->update([
            'refSurvHospi'       =>  $request->refSurvHospi,
            'dateDiab'    =>  $request->dateDiab,
            'glycemieMatin'    =>  $request->glycemieMatin,
            'doseMatin'       =>  $request->doseMatin,
    ...
    function delete_diabetique_hospie($id)
    {
        $data = thospi_diabetiquee_hospi::where('id',$id)->delete();
        return response()->json([
