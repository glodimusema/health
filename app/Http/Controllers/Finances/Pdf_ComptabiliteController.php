...
function printRapportJournal_Caisse($date1, $date2,$refTresorerie)
{
        ...
        ->get();
         ...
         ->select(DB::raw('IFNULL(ROUND(SUM(montantOpration),2),0) as totalDebit'))
         ->where([
            ['dateOpration','>=', $date1],
            ['dateOpration','<=', $date2],
         ...
         ->select(DB::raw('IFNULL(ROUND(SUM(montantOpration),2),0) as Credits'))
         ->where([
            ['dateOpration','>=', $date1],
            ['dateOpration','<=', $date2],
            ['refTresorerie','=', $refTresorerie],
...
function printRapportBilan($date1, $date2)
{
        ...
        ->get();
         ...
         ->select(DB::raw('IFNULL(ROUND(SUM(montant),2),0) as totalProduit'))
         ->where([
            ['dateOperation','>=', $date1],
            ['dateOperation','<=', $date2],
            ['numero_classe','=', '7']
         ...
         ->select(DB::raw('IFNULL(ROUND(SUM(montant),2),0) as charges'))
         ->where([
            ['dateOperation','>=', $date1],
            ['dateOperation','<=', $date2],
            ['numero_classe','=', '6']
        ...
        $data5 =   DB::select(
            'select (IFNULL(ROUND(SUM(montant),2),0) - :montants) as solde from tdepense  
...
function getInfoLivreCaisse($dateOperation)
{
           //Info Entreprise
            ...
            ->select(DB::raw('IFNULL(ROUND(SUM(montant),2),0) as totalEntree'))
            ->where([               
            ...
            ->select(DB::raw('IFNULL(ROUND(SUM(montant),2),0) as totalSortie'))
            ->where([               
                ['tdepense.dateOperation','<', $dateOperation],
                ['tdepense.refMvt','=', '2'],
                ['tconf_banque.nom_mode','=', 'CASH']
 ...
 //'tconf_banque.nom_mode','=', 'CASH'
             $data7 =   DB::select(
                 'select (IFNULL(ROUND(SUM(montant),2),0) - (:montants) + (:reports)) as solde_final from tdepense  
                  inner join tcompte on tcompte.id = tdepense.refCompte 
                  inner join ttypemouvement on ttypemouvement.id=tdepense.refMvt
                  inner join tconf_banque on tconf_banque.id = tdepense.refBanque 
                  inner join tfin_ssouscompte on tfin_ssouscompte.id = tconf_banque.refSscompte
            ...
            <table cellpadding="0" cellspacing="0" border="0" style="border-width:0px;empty-cells:show;width:949px;height:413px;position:relative;">
                ...
                <tr style="vertical-align:top;">
                    ...
                    <td></td>
                    <td class="cs7B44FCB" colspan="12" style="width:895px;height:44px;line-height:45px;text-align:center;vertical-align:middle;"><nobr>LIVRE&nbsp;DE&nbsp;CAISSE-HOPITAL</nobr></td>
                    <td></td>
                </tr>
                <tr style="vertical-align:top;">
...
function showDetailCaisse($dateOperation)
{
    ...
    foreach ($data as $row) 
    {
        ...
        ->select(DB::raw('IFNULL(ROUND(SUM(montant),2),0) as totEntree'))
        ->where([               
            ['tdepense.dateOperation','=', $dateOperation],
        ...
        ->select(DB::raw('IFNULL(ROUND(SUM(montant),2),0) as totSortie'))
        ->where([               
            ['tdepense.dateOperation','=', $dateOperation],
            ['tdepense.refBanque','=', $row->id],
            ['tdepense.refMvt','=', '2']
