...
class Pdf_FicheStockMedicamentController extends Controller
{
     /**
...
function pdf_fiche_stock_produit(Request $request)
{
...
function pdf_fiche_stock_produit_categorie(Request $request)
{
...
function getInfoFicheStock($date1,$date2)
{
           ...
           ->get();
            ...
            ->where([
                ['tmed_entete_vente.dateVente','>=', $date1],
                ['tmed_entete_vente.dateVente','<=', $date2]
            ])               
            ->get();
            ...
            <table cellpadding="0" cellspacing="0" border="0" style="border-width:0px;empty-cells:show;width:958px;height:352px;position:relative;">
                ...
                <tr style="vertical-align:top;">
                    ...
                    <td class="csF3AA49E4" style="width:76px;height:22px;line-height:15px;text-align:center;vertical-align:middle;"><nobr>VENTE</nobr></td>
                    ...
                    <td class="cs4B928201" colspan="3" style="width:139px;height:22px;line-height:15px;text-align:center;vertical-align:middle;"><nobr>SF</nobr></td>
...
function getInfoFicheStockCategorie($date1,$date2,$idCategorie)
{
           ...
           ->get();
            ...
            ->where([
                ['tmed_entete_vente.dateVente','>=', $date1],
                ['tmed_entete_vente.dateVente','<=', $date2],
                ['tconf_medicament.refcategoriemedicament','=', $idCategorie]
            ])               
            ->get();
...
function showCategorieFicheStockCategorie($date1,$date2,$idCategorie)
{
    ...
    "tconf_categoriemedicament.created_at")
    ->where([
        ['tconf_categoriemedicament.id','=', $idCategorie]
    ])
    ->orderBy("tconf_categoriemedicament.nom_categoriemedicament", "asc")
    ->get();
...
function showDetailFicheStock($date1,$date2,$refCategorie)
{
    ...
    foreach ($data1 as $row1) 
    {
        ...
        ->where([               
            ['tmed_entete_entree.dateEntree','<', $date1],
        ...
        $data12 = DB::table('tmed_detail_sortie')
        ->join('tconf_detailmedicament','tconf_detailmedicament.id','=','tmed_detail_sortie.refDetailMed')
        ->join('tconf_medicament','tconf_medicament.id','=','tconf_detailmedicament.refmedicament')
        ->join('tconf_categoriemedicament','tconf_categoriemedicament.id','=','tconf_medicament.refcategoriemedicament')
        ->join('tmed_entete_sortie','tmed_entete_sortie.id','=','tmed_detail_sortie.refEnteteSortie')
        ...
        $data4 =   DB::select(
            'select (IFNULL(ROUND(:quanteEntree,0),0) - IFNULL(ROUND(:quanteSortie,0),0)- IFNULL(ROUND(:quanteVente,0),0)) as SI from tconf_medicament  
             where (tconf_medicament.id = :idPro)',
             ['idPro' => $row1->id,'quanteEntree' => $totalEntree,'quanteSortie'=>$totalSortie,'quanteVente'=>$totalVente]
        );         
         ...
         $data5 = DB::table('tmed_detail_vente')
         ->join('tconf_detailmedicament','tconf_detailmedicament.id','=','tmed_detail_vente.refDetailMed')
         ->join('tconf_medicament','tconf_medicament.id','=','tconf_detailmedicament.refmedicament')
         ->join('tconf_categoriemedicament','tconf_categoriemedicament.id','=','tconf_medicament.refcategoriemedicament')
         ->join('tmed_entete_vente','tmed_entete_vente.id','=','tmed_detail_vente.refEnteteVente')
        ...
        $data13 = DB::table('tmed_detail_sortie')
        ->join('tconf_detailmedicament','tconf_detailmedicament.id','=','tmed_detail_sortie.refDetailMed')
        ->join('tconf_medicament','tconf_medicament.id','=','tconf_detailmedicament.refmedicament')
        ->join('tconf_categoriemedicament','tconf_categoriemedicament.id','=','tconf_medicament.refcategoriemedicament')
        ->join('tmed_entete_sortie','tmed_entete_sortie.id','=','tmed_detail_sortie.refEnteteSortie')
        ...
        $data6 = DB::table('tmed_detail_entree')
        ->join('tconf_medicament','tconf_medicament.id','=','tmed_detail_entree.refmedicament')
        ->join('tconf_categoriemedicament','tconf_categoriemedicament.id','=','tconf_medicament.refcategoriemedicament')
        ->join('tmed_entete_entree','tmed_entete_entree.id','=','tmed_detail_entree.refEnteteEntree')
        ->join('tfournisseur','tfournisseur.id','=','tmed_entete_entree.refFournisseur')
        ...
        ->where([
            ['tmed_entete_entree.dateEntree','>=', $date1],
            ['tmed_entete_entree.dateEntree','<=', $date2],
            ['tconf_medicament.id','=', $row1->id]
        ])
        ->get();        
        ...
        $data8 =   DB::select(
            'select (IFNULL(ROUND(:SI,0),0) + IFNULL(ROUND(:quanteEntree,0),0) - IFNULL(ROUND(:quanteSortie,0),0) - IFNULL(ROUND(:quanteVente,0),0)) as SF from tconf_medicament  
             where (tconf_medicament.id = :idPro)',
             ['idPro' => $row1->id,'SI' => $totalSI,'quanteEntree'=>$entreess,'quanteVente'=>$ventess,'quanteSortie'=>$sortiess]
        );         
...
//  
        $data9 = DB::table('tmed_detail_vente')
        ->join('tconf_detailmedicament','tconf_detailmedicament.id','=','tmed_detail_vente.refDetailMed')
        ->join('tconf_medicament','tconf_medicament.id','=','tconf_detailmedicament.refmedicament')
        ->join('tconf_categoriemedicament','tconf_categoriemedicament.id','=','tconf_medicament.refcategoriemedicament')
        ->join('tmed_entete_vente','tmed_entete_vente.id','=','tmed_detail_vente.refEnteteVente')
        ...
        ->join('provinces' , 'provinces.id','=','villes.idProvince')
        ->join('pays' , 'pays.id','=','provinces.idPays')
        ->select(DB::raw('IFNULL(ROUND(SUM(qteVente*puVente),0),0) as PTVente'))
        ->where([               
            ['tmed_entete_vente.dateVente','>=', $date1],
            ['tmed_entete_vente.dateVente','<=', $date2],
            ['tconf_medicament.id','=', $row1->id]
        ...
        $output .='
                ...
                <td class="csE78F4A6" style="width:76px;height:22px;line-height:15px;text-align:center;vertical-align:middle;">'.$ventess.'</td>
                <td class="cs4B928201" colspan="2" style="width:109px;height:22px;line-height:15px;text-align:center;vertical-align:middle;">'.$sortiess.'</td>
                <td class="cs4B928201" colspan="3" style="width:139px;height:22px;line-height:15px;text-align:center;vertical-align:middle;">'.$totalSF.'</td>
